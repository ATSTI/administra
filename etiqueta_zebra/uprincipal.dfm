object fPrincipal: TfPrincipal
  Left = 120
  Top = 52
  Width = 1083
  Height = 664
  BorderIcons = [biSystemMenu]
  Caption = 'Impress'#227'o de Etiquetas'
  Color = clGradientInactiveCaption
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label6: TLabel
    Left = 476
    Top = 2
    Width = 38
    Height = 16
    Caption = 'Marca'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label7: TLabel
    Left = 637
    Top = 1
    Width = 37
    Height = 16
    Caption = 'Grupo'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label8: TLabel
    Left = 812
    Top = 1
    Width = 59
    Height = 16
    Caption = 'Subgrupo'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    Visible = False
  end
  object Label1: TLabel
    Left = 88
    Top = 1
    Width = 62
    Height = 16
    Caption = 'Descri'#231#227'o'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label2: TLabel
    Left = 624
    Top = 337
    Width = 325
    Height = 65
    Caption = 
      'A 4251 A 4351 :Etiq.  21,2mm X 38,2 mm    -  5 Colunas  X 13 Lin' +
      'has'#13#10#13#10'A5Q - 2737      : Etiq.   27 mm  X  37 mm      -  5 Colun' +
      'as  X   5 Linhas'#13#10#13#10'6080                : Etiq. 25,4 mm  X  66,7' +
      ' mm -  3 Colunas   X 10 Linhas'
    Visible = False
  end
  object Label3: TLabel
    Left = 224
    Top = 250
    Width = 210
    Height = 20
    Caption = 'Etiquetas a Serem Impressas'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label4: TLabel
    Left = 729
    Top = 275
    Width = 99
    Height = 16
    Caption = 'Tipo de Etiqueta'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    Visible = False
  end
  object Label5: TLabel
    Left = 10
    Top = 2
    Width = 44
    Height = 16
    Caption = 'Codigo'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object lbl3: TLabel
    Left = 665
    Top = 423
    Width = 121
    Height = 13
    Caption = 'Qtd. Ja Utilizada na Folha'
    Visible = False
  end
  object Label9: TLabel
    Left = 976
    Top = 528
    Width = 60
    Height = 13
    Caption = 'Temperatura'
    Color = clBtnFace
    ParentColor = False
    Transparent = True
    Visible = False
  end
  object Label12: TLabel
    Left = 16
    Top = 16
    Width = 38
    Height = 13
    Caption = 'Modelo:'
    Color = clBtnFace
    ParentColor = False
  end
  object Label13: TLabel
    Left = 129
    Top = 16
    Width = 28
    Height = 13
    Caption = 'Porta:'
    Color = clBtnFace
    ParentColor = False
  end
  object Label14: TLabel
    Left = 800
    Top = 466
    Width = 38
    Height = 13
    Caption = 'Modelo:'
    Color = clBtnFace
    ParentColor = False
    Transparent = True
    Visible = False
  end
  object Label15: TLabel
    Left = 913
    Top = 466
    Width = 28
    Height = 13
    Caption = 'Porta:'
    Color = clBtnFace
    ParentColor = False
    Transparent = True
    Visible = False
  end
  object Label16: TLabel
    Left = 952
    Top = 421
    Width = 72
    Height = 13
    Caption = 'DPI Impressora'
    Color = clBtnFace
    ParentColor = False
    Transparent = True
    Visible = False
  end
  object Label10: TLabel
    Left = 8
    Top = 544
    Width = 163
    Height = 20
    Caption = 'ATS - www.atsti.com.br'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Memo1: TMemo
    Left = 616
    Top = 248
    Width = 433
    Height = 313
    Lines.Strings = (
      'Imprimir Etiqueta')
    TabOrder = 25
  end
  object cbMarca: TComboBox
    Left = 473
    Top = 18
    Width = 161
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ItemHeight = 16
    ParentFont = False
    TabOrder = 0
    OnChange = cbMarcaChange
  end
  object cbFamilia: TComboBox
    Left = 638
    Top = 17
    Width = 169
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ItemHeight = 16
    ParentFont = False
    TabOrder = 1
    OnChange = cbFamiliaChange
  end
  object cbCategoria: TComboBox
    Left = 811
    Top = 17
    Width = 152
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ItemHeight = 16
    ParentFont = False
    TabOrder = 2
    Visible = False
  end
  object BitBtn1: TBitBtn
    Left = 119
    Top = 472
    Width = 90
    Height = 55
    Caption = 'F8-Procurar'
    TabOrder = 3
    TabStop = False
    OnClick = BitBtn1Click
    Glyph.Data = {
      
      C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
    Layout = blGlyphTop
  end
  object SpeedButton1: TBitBtn
    Left = 316
    Top = 528
    Width = 90
    Height = 55
    Caption = 'Etiqueta Pimaco'
    TabOrder = 4
    TabStop = False
    Visible = False
    OnClick = SpeedButton1Click
    Glyph.Data = {
      
      C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
    Layout = blGlyphTop
  end
  object EvDBFind1: TEvDBFind
    Left = 87
    Top = 17
    Width = 384
    Height = 24
    CharCase = ecUpperCase
    DataField = 'PRODUTO'
    DataSource = Dtsrc
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 5
  end
  object DBGrid1: TDBGrid
    Left = 7
    Top = 273
    Width = 609
    Height = 188
    Color = clCream
    DataSource = DSq
    Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 6
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'CODPROD'
        Title.Caption = 'Codigo'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PRODUTO'
        Title.Caption = 'Descri'#231#227'o'
        Width = 424
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'QTD'
        Title.Caption = 'QTDE'
        Visible = True
      end>
  end
  object SpeedButton2: TBitBtn
    Left = 304
    Top = 472
    Width = 90
    Height = 55
    Caption = 'Etiquetas'
    TabOrder = 7
    TabStop = False
    OnClick = SpeedButton2Click
    Glyph.Data = {
      
      1010101010070707070707070707070707070707070707070707}
    Layout = blGlyphTop
  end
  object DBGrid2: TDBGrid
    Left = 6
    Top = 48
    Width = 955
    Height = 188
    DataSource = Dtsrc
    TabOrder = 8
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'CODPRO'
        ReadOnly = True
        Title.Caption = 'Codigo'
        Width = 102
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PRODUTO'
        ReadOnly = True
        Title.Caption = 'Descri'#231#227'o'
        Width = 635
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'QTD'
        Title.Caption = 'QTDE'
        Visible = True
      end>
  end
  object Combo: TComboBox
    Left = 712
    Top = 294
    Width = 145
    Height = 21
    ItemHeight = 13
    TabOrder = 9
    Visible = False
    Items.Strings = (
      'A 4251 A 4351'
      'A5Q-2337'
      '6080'
      '')
  end
  object EvDBFind2: TEvDBFind
    Left = 7
    Top = 16
    Width = 77
    Height = 24
    DataField = 'CODPRO'
    DataSource = Dtsrc
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 10
  end
  object BitBtn3: TBitBtn
    Left = 632
    Top = 248
    Width = 75
    Height = 25
    Caption = 'BitBtn3'
    TabOrder = 11
    Visible = False
  end
  object edt1: TEdit
    Left = 623
    Top = 420
    Width = 35
    Height = 21
    TabOrder = 12
    Text = '0'
    Visible = False
  end
  object btn1Coluna: TBitBtn
    Left = 504
    Top = 536
    Width = 90
    Height = 55
    Caption = '1 Coluna'
    TabOrder = 13
    TabStop = False
    Visible = False
    OnClick = btn1ColunaClick
    Glyph.Data = {
      
      C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
    Layout = blGlyphTop
  end
  object btnSair: TBitBtn
    Left = 998
    Top = 568
    Width = 70
    Height = 55
    Caption = 'F9-Sair'
    TabOrder = 14
    OnClick = btnSairClick
    Glyph.Data = {
      
      C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
    Layout = blGlyphTop
  end
  object btnExcluir: TBitBtn
    Left = 212
    Top = 472
    Width = 90
    Height = 55
    Hint = 'Limpa a Lista das Etiquetas Selecionadas '#13#10'Para ser Impressa.'
    Caption = 'Limpar'
    ParentShowHint = False
    ShowHint = True
    TabOrder = 15
    OnClick = btnExcluirClick
    Glyph.Data = {
      
      0000}
    Layout = blGlyphTop
  end
  object bEtqSimples: TButton
    Left = 524
    Top = 471
    Width = 88
    Height = 56
    Caption = 'IMPRIMIR'
    TabOrder = 16
    OnClick = bEtqSimplesClick
  end
  object cbModelo: TComboBox
    Left = 800
    Top = 482
    Width = 90
    Height = 21
    Style = csDropDownList
    ItemHeight = 13
    ItemIndex = 2
    TabOrder = 17
    Text = 'etqPPlb'
    Visible = False
    Items.Strings = (
      'etqNenhuma'
      'etqPpla'
      'etqPPlb'
      'etqZPLII'
      'etqEpl2')
  end
  object cbPorta: TComboBox
    Left = 913
    Top = 481
    Width = 144
    Height = 21
    ItemHeight = 13
    TabOrder = 18
    Text = 'LPT1'
    Visible = False
    Items.Strings = (
      'LPT1'
      '\\localhost\TLP_2844'
      'LPT1'
      'LPT2'
      'COM1'
      'COM2'
      'COM3'
      '\\localhost\ZEBRA'
      '\\127.0.0.1\ARGOX'
      'Digite a porta')
  end
  object eAvanco1: TEdit
    Left = 976
    Top = 544
    Width = 65
    Height = 21
    TabOrder = 19
    Text = '5'
    Visible = False
  end
  object cbDPI: TComboBox
    Left = 952
    Top = 438
    Width = 90
    Height = 21
    Style = csDropDownList
    ItemHeight = 13
    ItemIndex = 0
    TabOrder = 20
    Text = 'dpi203'
    Visible = False
    Items.Strings = (
      'dpi203'
      'dpi300'
      'dpi600')
  end
  object BitBtn5: TBitBtn
    Left = 603
    Top = 572
    Width = 286
    Height = 25
    Caption = 'Etiqueta Zebra -Como : Trocar Tipo de Etiqueta / Calibrar'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 21
    Visible = False
    OnClick = BitBtn5Click
  end
  object BitBtn4: TBitBtn
    Left = 432
    Top = 544
    Width = 75
    Height = 25
    Caption = 'ZEBRA'
    TabOrder = 22
    Visible = False
    OnClick = BitBtn4Click
  end
  object Button1: TButton
    Left = 523
    Top = 471
    Width = 88
    Height = 56
    Caption = 'IMPRIMIR'
    TabOrder = 23
    OnClick = Button1Click
  end
  object btnImprimeIza: TButton
    Left = 627
    Top = 567
    Width = 118
    Height = 56
    Caption = 'IMP. ARQUIVO - IZA'
    TabOrder = 24
    OnClick = btnImprimeIzaClick
  end
  object Edit1: TEdit
    Left = 16
    Top = 576
    Width = 153
    Height = 21
    TabOrder = 26
    Text = 'LPT1'
  end
  object edDiretorio: TEdit
    Left = 16
    Top = 608
    Width = 121
    Height = 21
    TabOrder = 27
    Text = 'C:\home'
  end
  object SQLPro: TSQLDataSet
    CommandText = 'select  * from PRODUTOS '
    MaxBlobSize = -1
    Params = <>
    SQLConnection = sqlCon
    Left = 344
    Top = 7
    object SQLProCODPRODUTO: TIntegerField
      FieldName = 'CODPRODUTO'
      Required = True
    end
    object SQLProFAMILIA: TStringField
      FieldName = 'FAMILIA'
      Size = 30
    end
    object SQLProCATEGORIA: TStringField
      FieldName = 'CATEGORIA'
      Size = 30
    end
    object SQLProMARCA: TStringField
      FieldName = 'MARCA'
      Size = 30
    end
    object SQLProUNIDADEMEDIDA: TStringField
      FieldName = 'UNIDADEMEDIDA'
      FixedChar = True
      Size = 2
    end
    object SQLProDATAULTIMACOMPRA: TDateField
      FieldName = 'DATAULTIMACOMPRA'
    end
    object SQLProESTOQUEMAXIMO: TFloatField
      FieldName = 'ESTOQUEMAXIMO'
    end
    object SQLProESTOQUEATUAL: TFloatField
      FieldName = 'ESTOQUEATUAL'
    end
    object SQLProESTOQUEREPOSICAO: TFloatField
      FieldName = 'ESTOQUEREPOSICAO'
    end
    object SQLProESTOQUEMINIMO: TFloatField
      FieldName = 'ESTOQUEMINIMO'
    end
    object SQLProVALORUNITARIOATUAL: TFloatField
      FieldName = 'VALORUNITARIOATUAL'
    end
    object SQLProVALORUNITARIOANTERIOR: TFloatField
      FieldName = 'VALORUNITARIOANTERIOR'
    end
    object SQLProICMS: TFloatField
      FieldName = 'ICMS'
    end
    object SQLProCODALMOXARIFADO: TIntegerField
      FieldName = 'CODALMOXARIFADO'
    end
    object SQLProIPI: TFloatField
      FieldName = 'IPI'
    end
    object SQLProCLASSIFIC_FISCAL: TStringField
      FieldName = 'CLASSIFIC_FISCAL'
      Size = 30
    end
    object SQLProCST: TStringField
      FieldName = 'CST'
      Size = 30
    end
    object SQLProBASE_ICMS: TFloatField
      FieldName = 'BASE_ICMS'
    end
    object SQLProPRODUTO: TStringField
      FieldName = 'PRODUTO'
      Required = True
      Size = 300
    end
    object SQLProPRECOMEDIO: TBCDField
      FieldName = 'PRECOMEDIO'
      Precision = 9
      Size = 2
    end
    object SQLProCOD_COMISSAO: TIntegerField
      FieldName = 'COD_COMISSAO'
    end
    object SQLProMARGEM_LUCRO: TFloatField
      FieldName = 'MARGEM_LUCRO'
    end
    object SQLProCOD_BARRA: TStringField
      FieldName = 'COD_BARRA'
    end
    object SQLProVALOR_PRAZO: TFloatField
      FieldName = 'VALOR_PRAZO'
    end
    object SQLProTIPO: TStringField
      FieldName = 'TIPO'
      Size = 10
    end
    object SQLProCONTA_DESPESA: TStringField
      FieldName = 'CONTA_DESPESA'
      Size = 15
    end
    object SQLProCONTA_RECEITA: TStringField
      FieldName = 'CONTA_RECEITA'
      Size = 15
    end
    object SQLProCONTA_ESTOQUE: TStringField
      FieldName = 'CONTA_ESTOQUE'
      Size = 15
    end
    object SQLProRATEIO: TStringField
      FieldName = 'RATEIO'
      FixedChar = True
      Size = 1
    end
    object SQLProCODPRO: TStringField
      FieldName = 'CODPRO'
      Size = 15
    end
    object SQLProQTDE_PCT: TFloatField
      FieldName = 'QTDE_PCT'
    end
    object SQLProPESO_QTDE: TFloatField
      FieldName = 'PESO_QTDE'
    end
    object SQLProDATACADASTRO: TSQLTimeStampField
      FieldName = 'DATACADASTRO'
    end
    object SQLProMARGEM: TFloatField
      FieldName = 'MARGEM'
    end
    object SQLProPRO_COD: TStringField
      FieldName = 'PRO_COD'
      Size = 15
    end
    object SQLProDATAGRAV: TDateField
      FieldName = 'DATAGRAV'
    end
    object SQLProCODFORN: TStringField
      FieldName = 'CODFORN'
      Size = 60
    end
    object SQLProFOTOPRODUTO: TStringField
      FieldName = 'FOTOPRODUTO'
      Size = 80
    end
    object SQLProLOTES: TStringField
      FieldName = 'LOTES'
      FixedChar = True
      Size = 1
    end
    object SQLProUSA: TStringField
      FieldName = 'USA'
      Size = 3
    end
    object SQLProLOCALIZACAO: TStringField
      FieldName = 'LOCALIZACAO'
      Size = 50
    end
    object SQLProTIPOPRECOVENDA: TStringField
      FieldName = 'TIPOPRECOVENDA'
      FixedChar = True
      Size = 1
    end
    object SQLProVALORMINIMO: TFloatField
      FieldName = 'VALORMINIMO'
    end
    object SQLProVALORCOMISSAO: TFloatField
      FieldName = 'VALORCOMISSAO'
    end
    object SQLProGERADESCONTO: TStringField
      FieldName = 'GERADESCONTO'
      FixedChar = True
      Size = 1
    end
    object SQLProIMPRIMIR: TStringField
      FieldName = 'IMPRIMIR'
      FixedChar = True
      Size = 1
    end
    object SQLProORIGEM: TIntegerField
      FieldName = 'ORIGEM'
    end
    object SQLProNCM: TStringField
      FieldName = 'NCM'
      Size = 8
    end
    object SQLProIMPRESSORA_1: TStringField
      FieldName = 'IMPRESSORA_1'
      Size = 10
    end
    object SQLProIMPRESSORA_2: TStringField
      FieldName = 'IMPRESSORA_2'
      Size = 10
    end
    object SQLProIMPRESSORA_3: TStringField
      FieldName = 'IMPRESSORA_3'
      Size = 10
    end
    object SQLProQTD: TIntegerField
      FieldName = 'QTD'
    end
  end
  object CDSPro: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPPro'
    Left = 456
    Top = 7
    object CDSProCODPRODUTO: TIntegerField
      DisplayWidth = 16
      FieldName = 'CODPRODUTO'
      Required = True
    end
    object CDSProFAMILIA: TStringField
      DisplayWidth = 36
      FieldName = 'FAMILIA'
      Size = 30
    end
    object CDSProCATEGORIA: TStringField
      DisplayWidth = 36
      FieldName = 'CATEGORIA'
      Size = 30
    end
    object CDSProMARCA: TStringField
      DisplayWidth = 36
      FieldName = 'MARCA'
      Size = 30
    end
    object CDSProUNIDADEMEDIDA: TStringField
      DisplayWidth = 18
      FieldName = 'UNIDADEMEDIDA'
      FixedChar = True
      Size = 2
    end
    object CDSProDATAULTIMACOMPRA: TDateField
      DisplayWidth = 23
      FieldName = 'DATAULTIMACOMPRA'
    end
    object CDSProESTOQUEMAXIMO: TFloatField
      DisplayWidth = 19
      FieldName = 'ESTOQUEMAXIMO'
    end
    object CDSProESTOQUEATUAL: TFloatField
      DisplayWidth = 18
      FieldName = 'ESTOQUEATUAL'
    end
    object CDSProESTOQUEREPOSICAO: TFloatField
      DisplayWidth = 23
      FieldName = 'ESTOQUEREPOSICAO'
    end
    object CDSProESTOQUEMINIMO: TFloatField
      DisplayWidth = 19
      FieldName = 'ESTOQUEMINIMO'
    end
    object CDSProVALORUNITARIOATUAL: TFloatField
      DisplayWidth = 25
      FieldName = 'VALORUNITARIOATUAL'
    end
    object CDSProVALORUNITARIOANTERIOR: TFloatField
      DisplayWidth = 29
      FieldName = 'VALORUNITARIOANTERIOR'
    end
    object CDSProICMS: TFloatField
      DisplayWidth = 12
      FieldName = 'ICMS'
    end
    object CDSProCODALMOXARIFADO: TIntegerField
      DisplayWidth = 22
      FieldName = 'CODALMOXARIFADO'
    end
    object CDSProIPI: TFloatField
      DisplayWidth = 12
      FieldName = 'IPI'
    end
    object CDSProCLASSIFIC_FISCAL: TStringField
      DisplayWidth = 36
      FieldName = 'CLASSIFIC_FISCAL'
      Size = 30
    end
    object CDSProCST: TStringField
      DisplayWidth = 36
      FieldName = 'CST'
      Size = 30
    end
    object CDSProBASE_ICMS: TFloatField
      DisplayWidth = 12
      FieldName = 'BASE_ICMS'
    end
    object CDSProPRODUTO: TStringField
      DisplayWidth = 360
      FieldName = 'PRODUTO'
      Required = True
      Size = 300
    end
    object CDSProPRECOMEDIO: TBCDField
      DisplayWidth = 15
      FieldName = 'PRECOMEDIO'
      Precision = 9
      Size = 2
    end
    object CDSProCOD_COMISSAO: TIntegerField
      DisplayWidth = 17
      FieldName = 'COD_COMISSAO'
    end
    object CDSProMARGEM_LUCRO: TFloatField
      DisplayWidth = 18
      FieldName = 'MARGEM_LUCRO'
    end
    object CDSProCOD_BARRA: TStringField
      DisplayWidth = 24
      FieldName = 'COD_BARRA'
    end
    object CDSProVALOR_PRAZO: TFloatField
      DisplayWidth = 16
      FieldName = 'VALOR_PRAZO'
    end
    object CDSProTIPO: TStringField
      DisplayWidth = 12
      FieldName = 'TIPO'
      Size = 10
    end
    object CDSProCONTA_DESPESA: TStringField
      DisplayWidth = 19
      FieldName = 'CONTA_DESPESA'
      Size = 15
    end
    object CDSProCONTA_RECEITA: TStringField
      DisplayWidth = 18
      FieldName = 'CONTA_RECEITA'
      Size = 15
    end
    object CDSProCONTA_ESTOQUE: TStringField
      DisplayWidth = 19
      FieldName = 'CONTA_ESTOQUE'
      Size = 15
    end
    object CDSProRATEIO: TStringField
      DisplayWidth = 8
      FieldName = 'RATEIO'
      FixedChar = True
      Size = 1
    end
    object CDSProCODPRO: TStringField
      DisplayWidth = 18
      FieldName = 'CODPRO'
      Size = 15
    end
    object CDSProQTDE_PCT: TFloatField
      DisplayWidth = 12
      FieldName = 'QTDE_PCT'
    end
    object CDSProPESO_QTDE: TFloatField
      DisplayWidth = 13
      FieldName = 'PESO_QTDE'
    end
    object CDSProDATACADASTRO: TSQLTimeStampField
      DisplayWidth = 41
      FieldName = 'DATACADASTRO'
    end
    object CDSProMARGEM: TFloatField
      DisplayWidth = 12
      FieldName = 'MARGEM'
    end
    object CDSProPRO_COD: TStringField
      DisplayWidth = 18
      FieldName = 'PRO_COD'
      Size = 15
    end
    object CDSProDATAGRAV: TDateField
      DisplayWidth = 12
      FieldName = 'DATAGRAV'
    end
    object CDSProCODFORN: TStringField
      DisplayWidth = 72
      FieldName = 'CODFORN'
      Size = 60
    end
    object CDSProFOTOPRODUTO: TStringField
      DisplayWidth = 96
      FieldName = 'FOTOPRODUTO'
      Size = 80
    end
    object CDSProLOTES: TStringField
      DisplayWidth = 7
      FieldName = 'LOTES'
      FixedChar = True
      Size = 1
    end
    object CDSProUSA: TStringField
      DisplayWidth = 5
      FieldName = 'USA'
      Size = 3
    end
    object CDSProLOCALIZACAO: TStringField
      DisplayWidth = 60
      FieldName = 'LOCALIZACAO'
      Size = 50
    end
    object CDSProTIPOPRECOVENDA: TStringField
      DisplayWidth = 20
      FieldName = 'TIPOPRECOVENDA'
      FixedChar = True
      Size = 1
    end
    object CDSProVALORMINIMO: TFloatField
      DisplayWidth = 15
      FieldName = 'VALORMINIMO'
    end
    object CDSProVALORCOMISSAO: TFloatField
      DisplayWidth = 19
      FieldName = 'VALORCOMISSAO'
    end
    object CDSProGERADESCONTO: TStringField
      DisplayWidth = 18
      FieldName = 'GERADESCONTO'
      FixedChar = True
      Size = 1
    end
    object CDSProIMPRIMIR: TStringField
      DisplayWidth = 13
      FieldName = 'IMPRIMIR'
      FixedChar = True
      Size = 1
    end
    object CDSProORIGEM: TIntegerField
      DisplayWidth = 12
      FieldName = 'ORIGEM'
    end
    object CDSProNCM: TStringField
      DisplayWidth = 10
      FieldName = 'NCM'
      Size = 8
    end
    object CDSProIMPRESSORA_1: TStringField
      FieldName = 'IMPRESSORA_1'
      Size = 10
    end
    object CDSProIMPRESSORA_2: TStringField
      FieldName = 'IMPRESSORA_2'
      Size = 10
    end
    object CDSProIMPRESSORA_3: TStringField
      FieldName = 'IMPRESSORA_3'
      Size = 10
    end
    object CDSProQTD: TIntegerField
      FieldName = 'QTD'
    end
  end
  object DSPPro: TDataSetProvider
    DataSet = SQLPro
    Left = 408
    Top = 7
  end
  object DSPro: TDataSource
    DataSet = CDSPro
    Left = 504
    Top = 7
  end
  object SQLD1: TSQLDataSet
    CommandText = 'select * from ETIQUETA'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = sqlCon
    Left = 352
    Top = 455
    object SQLD1CODPRODUTO: TIntegerField
      FieldName = 'CODPRODUTO'
    end
    object SQLD1QTD: TIntegerField
      FieldName = 'QTD'
    end
    object SQLD1PRODUTO: TStringField
      FieldName = 'PRODUTO'
      Size = 300
    end
    object SQLD1CODPROD: TStringField
      FieldName = 'CODPROD'
      Size = 15
    end
    object SQLD1LOCALIZACAO: TStringField
      FieldName = 'LOCALIZACAO'
      Size = 50
    end
    object SQLD1ORIGEM: TIntegerField
      FieldName = 'ORIGEM'
    end
  end
  object CDS1: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSP1'
    Left = 464
    Top = 456
    object CDS1CODPRODUTO: TIntegerField
      FieldName = 'CODPRODUTO'
    end
    object CDS1QTD: TIntegerField
      FieldName = 'QTD'
    end
    object CDS1PRODUTO: TStringField
      FieldName = 'PRODUTO'
      Size = 300
    end
    object CDS1CODPROD: TStringField
      FieldName = 'CODPROD'
      Size = 15
    end
    object CDS1LOCALIZACAO: TStringField
      FieldName = 'LOCALIZACAO'
      Size = 50
    end
    object CDS1ORIGEM: TIntegerField
      FieldName = 'ORIGEM'
    end
  end
  object DSP1: TDataSetProvider
    DataSet = SQLD1
    Left = 416
    Top = 456
  end
  object DS1: TDataSource
    DataSet = CDS1
    Left = 512
    Top = 456
  end
  object cds_proc: TClientDataSet
    Aggregates = <>
    PacketRecords = 30
    Params = <>
    ProviderName = 'dsp_proc'
    Left = 104
    Top = 16
    object cds_procCODPRODUTO: TIntegerField
      FieldName = 'CODPRODUTO'
      Required = True
    end
    object cds_procCOD_BARRA: TStringField
      FieldName = 'COD_BARRA'
    end
    object cds_procCODPRO: TStringField
      FieldName = 'CODPRO'
      Size = 15
    end
    object cds_procPRODUTO: TStringField
      FieldName = 'PRODUTO'
      Required = True
      Size = 300
    end
    object cds_procFAMILIA: TStringField
      FieldName = 'FAMILIA'
      Size = 30
    end
    object cds_procQTDE_PCT: TFloatField
      FieldName = 'QTDE_PCT'
    end
    object cds_procUNIDADEMEDIDA: TStringField
      FieldName = 'UNIDADEMEDIDA'
      FixedChar = True
      Size = 2
    end
    object cds_procMARCA: TStringField
      FieldName = 'MARCA'
      Size = 30
    end
    object cds_procESTOQUEATUAL: TFloatField
      FieldName = 'ESTOQUEATUAL'
    end
    object cds_procCODALMOXARIFADO: TIntegerField
      FieldName = 'CODALMOXARIFADO'
    end
    object cds_procICMS: TFloatField
      FieldName = 'ICMS'
    end
    object cds_procTIPO: TStringField
      FieldName = 'TIPO'
      Size = 10
    end
    object cds_procLOCALIZACAO: TStringField
      FieldName = 'LOCALIZACAO'
      Size = 50
    end
    object cds_procLOTES: TStringField
      FieldName = 'LOTES'
      FixedChar = True
      Size = 1
    end
    object cds_procCATEGORIA: TStringField
      FieldName = 'CATEGORIA'
      Size = 30
    end
    object cds_procPESO_QTDE: TFloatField
      FieldName = 'PESO_QTDE'
    end
    object cds_procIPI: TFloatField
      FieldName = 'IPI'
    end
    object cds_procORIGEM: TIntegerField
      FieldName = 'ORIGEM'
    end
    object cds_procQTD: TIntegerField
      FieldName = 'QTD'
    end
  end
  object dsp_proc: TDataSetProvider
    DataSet = sds_proc
    Options = [poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 72
    Top = 16
  end
  object sds_proc: TSQLDataSet
    CommandText = 
      'select  CODPRODUTO, COD_BARRA, CODPRO,  PRODUTO,FAMILIA ,QTDE_PC' +
      'T, UNIDADEMEDIDA, MARCA, ESTOQUEATUAL, CODALMOXARIFADO, ICMS, TI' +
      'PO, LOCALIZACAO, LOTES, CATEGORIA, PESO_QTDE, IPI ,origem , qtd ' +
      #13#10#13#10#13#10'from PRODUTOS'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = sqlCon
    Left = 32
    Top = 16
    object sds_procCODPRODUTO: TIntegerField
      FieldName = 'CODPRODUTO'
      Required = True
    end
    object sds_procCOD_BARRA: TStringField
      FieldName = 'COD_BARRA'
    end
    object sds_procCODPRO: TStringField
      FieldName = 'CODPRO'
      Size = 15
    end
    object sds_procPRODUTO: TStringField
      FieldName = 'PRODUTO'
      Required = True
      Size = 300
    end
    object sds_procFAMILIA: TStringField
      FieldName = 'FAMILIA'
      Size = 30
    end
    object sds_procQTDE_PCT: TFloatField
      FieldName = 'QTDE_PCT'
    end
    object sds_procUNIDADEMEDIDA: TStringField
      FieldName = 'UNIDADEMEDIDA'
      FixedChar = True
      Size = 2
    end
    object sds_procMARCA: TStringField
      FieldName = 'MARCA'
      Size = 30
    end
    object sds_procESTOQUEATUAL: TFloatField
      FieldName = 'ESTOQUEATUAL'
    end
    object sds_procCODALMOXARIFADO: TIntegerField
      FieldName = 'CODALMOXARIFADO'
    end
    object sds_procICMS: TFloatField
      FieldName = 'ICMS'
    end
    object sds_procTIPO: TStringField
      FieldName = 'TIPO'
      Size = 10
    end
    object sds_procLOCALIZACAO: TStringField
      FieldName = 'LOCALIZACAO'
      Size = 50
    end
    object sds_procLOTES: TStringField
      FieldName = 'LOTES'
      FixedChar = True
      Size = 1
    end
    object sds_procCATEGORIA: TStringField
      FieldName = 'CATEGORIA'
      Size = 30
    end
    object sds_procPESO_QTDE: TFloatField
      FieldName = 'PESO_QTDE'
    end
    object sds_procIPI: TFloatField
      FieldName = 'IPI'
    end
    object sds_procORIGEM: TIntegerField
      FieldName = 'ORIGEM'
    end
    object sds_procQTD: TIntegerField
      FieldName = 'QTD'
    end
  end
  object Dtsrc: TDataSource
    DataSet = cds_proc
    Left = 144
    Top = 16
  end
  object SQLq: TSQLDataSet
    CommandText = 'select distinct * from ETIQUETA'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = sqlCon
    Left = 104
    Top = 463
    object SQLqCODPRODUTO: TIntegerField
      FieldName = 'CODPRODUTO'
    end
    object SQLqQTD: TIntegerField
      FieldName = 'QTD'
    end
    object SQLqPRODUTO: TStringField
      FieldName = 'PRODUTO'
      Size = 300
    end
    object SQLqCODPROD: TStringField
      FieldName = 'CODPROD'
      Size = 15
    end
    object SQLqLOCALIZACAO: TStringField
      FieldName = 'LOCALIZACAO'
      Size = 50
    end
    object SQLqORIGEM: TIntegerField
      FieldName = 'ORIGEM'
    end
    object SQLqVALOR_PRAZO: TFloatField
      FieldName = 'VALOR_PRAZO'
    end
    object SQLqCOD_BARRA: TStringField
      FieldName = 'COD_BARRA'
    end
    object SQLqOBS: TStringField
      FieldName = 'OBS'
      Size = 300
    end
  end
  object CDSq: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPq'
    Left = 184
    Top = 464
    object CDSqCODPRODUTO: TIntegerField
      FieldName = 'CODPRODUTO'
    end
    object CDSqQTD: TIntegerField
      FieldName = 'QTD'
    end
    object CDSqPRODUTO: TStringField
      FieldName = 'PRODUTO'
      Size = 300
    end
    object CDSqCODPROD: TStringField
      FieldName = 'CODPROD'
      Size = 15
    end
    object CDSqLOCALIZACAO: TStringField
      FieldName = 'LOCALIZACAO'
      Size = 50
    end
    object CDSqORIGEM: TIntegerField
      FieldName = 'ORIGEM'
    end
    object CDSqVALOR_PRAZO: TFloatField
      FieldName = 'VALOR_PRAZO'
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object CDSqCOD_BARRA: TStringField
      FieldName = 'COD_BARRA'
    end
    object CDSqOBS: TStringField
      FieldName = 'OBS'
      Size = 300
    end
  end
  object DSPq: TDataSetProvider
    DataSet = SQLq
    Left = 144
    Top = 464
  end
  object DSq: TDataSource
    DataSet = CDSq
    Left = 216
    Top = 464
  end
  object VCLReport1: TVCLReport
    AsyncExecution = False
    Title = 'Untitled'
    Left = 232
    Top = 8
  end
  object sqlCon: TSQLConnection
    ConnectionName = 'sisAdmin'
    DriverName = 'UIB FireBird15'
    GetDriverFunc = 'getSQLDriverINTERBASE'
    LibraryName = 'dbexpUIBfire15.dll'
    LoginPrompt = False
    Params.Strings = (
      'DriverName=UIB FireBird15'
      'BlobSize=-1'
      'CommitRetain=False'
      'Database=\\XPVIR\home\bd\sge_felicita.fdb'
      'ErrorResourceFile=\\XPVIR\home\bd\SGE_ATS.FDB'
      'LocaleCode=0000'
      'Password=masterkey'
      'RoleName=RoleName'
      'ServerCharSet=win1252'
      'SQLDialect=3'
      'Interbase TransIsolation=ReadCommited'
      'User_Name=sysdba'
      'WaitOnLocks=True'
      'str_relatorio=C:\home\sisadmin\relatorio\')
    VendorLib = 'fbclient.dll'
    Left = 24
    Top = 472
  end
  object cds_Marca: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dsp_Marca'
    Left = 991
    Top = 176
    object cds_MarcaDESCMARCAS: TStringField
      FieldName = 'DESCMARCAS'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 30
    end
  end
  object dsp_Marca: TDataSetProvider
    DataSet = sds_Marca
    UpdateMode = upWhereKeyOnly
    Left = 922
    Top = 168
  end
  object sds_Marca: TSQLDataSet
    CommandText = 'select * from MARCA order by DESCMARCAS'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = sqlCon
    Left = 854
    Top = 168
    object sds_MarcaDESCMARCAS: TStringField
      FieldName = 'DESCMARCAS'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 30
    end
  end
  object cds_familia: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dsp_familia'
    Left = 991
    Top = 224
    object cds_familiaDESCFAMILIA: TStringField
      FieldName = 'DESCFAMILIA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 30
    end
    object cds_familiaCOD_FAMILIA: TIntegerField
      FieldName = 'COD_FAMILIA'
    end
    object cds_familiaMARCA: TStringField
      FieldName = 'MARCA'
      Size = 30
    end
  end
  object dsp_familia: TDataSetProvider
    DataSet = sds_familia
    Options = [poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 922
    Top = 216
  end
  object sds_familia: TSQLDataSet
    CommandText = 'select * from FAMILIAPRODUTOS  order by DESCFAMILIA'#13#10
    MaxBlobSize = -1
    Params = <>
    SQLConnection = sqlCon
    Left = 854
    Top = 216
    object sds_familiaDESCFAMILIA: TStringField
      FieldName = 'DESCFAMILIA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 30
    end
    object sds_familiaCOD_FAMILIA: TIntegerField
      FieldName = 'COD_FAMILIA'
    end
    object sds_familiaMARCA: TStringField
      FieldName = 'MARCA'
      Size = 30
    end
  end
  object dsp_categoria: TDataSetProvider
    DataSet = sds_categoria
    UpdateMode = upWhereKeyOnly
    Left = 939
    Top = 280
  end
  object cds_categoria: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'pcod'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'pcod'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'PFAM'
        ParamType = ptInput
      end>
    ProviderName = 'dsp_categoria'
    Left = 1008
    Top = 280
    object cds_categoriaDESCCATEGORIA: TStringField
      FieldName = 'DESCCATEGORIA'
      Size = 30
    end
    object cds_categoriaCOD_CATEGORIA: TIntegerField
      FieldName = 'COD_CATEGORIA'
    end
    object cds_categoriaCOD_FAMILIA: TIntegerField
      FieldName = 'COD_FAMILIA'
      Required = True
    end
    object cds_categoriaCASE: TStringField
      FieldName = 'CASE'
      Size = 254
    end
  end
  object sds_categoria: TSQLDataSet
    CommandText = 
      'select COD_CATEGORIA, COD_FAMILIA, DESCCATEGORIA, '#13#10'CASE UDF_ISD' +
      'IGIT(DESCCATEGORIA) when 1 then (UDF_RIGHT(DESCCATEGORIA, UDF_LE' +
      'N(DESCCATEGORIA)-9)) '#13#10'when 0 then DESCCATEGORIA end '#13#10' from CAT' +
      'EGORIAPRODUTO '#13#10'where ((DESCCATEGORIA=:pcod) or (:pcod = '#39'todos'#39 +
      ')) or (COD_FAMILIA = :PFAM) order by DESCCATEGORIA'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'pcod'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'pcod'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'PFAM'
        ParamType = ptInput
      end>
    SQLConnection = sqlCon
    Left = 870
    Top = 280
    object sds_categoriaDESCCATEGORIA: TStringField
      FieldName = 'DESCCATEGORIA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 30
    end
    object sds_categoriaCOD_CATEGORIA: TIntegerField
      FieldName = 'COD_CATEGORIA'
    end
    object sds_categoriaCOD_FAMILIA: TIntegerField
      FieldName = 'COD_FAMILIA'
      Required = True
    end
    object sds_categoriaCASE: TStringField
      FieldName = 'CASE'
      ProviderFlags = []
      Size = 254
    end
  end
  object ACBrETQ: TACBrETQ
    Porta = 'LPT1'
    Left = 880
    Top = 420
  end
end

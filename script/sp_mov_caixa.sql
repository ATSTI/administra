...
BEGIN
  ...
  BEGIN
    SELECT CONTA, NOME FROM PLANO WHERE CODIGO = :COD_CAIXA
      INTO :NCONTA, :CAIXA;
    select SUM(COALESCE(VALORCREDITO,0)), SUM(COALESCE(VALORDEBITO,0)) from MOVIMENTOCONT where data < :DTAINI and tipoorigem = 'CONTABIL' and CONTA = :NCONTA
      INTO :VLINIC, VLINID; 
  END
  ELSE
  BEGIN 
    select SUM(COALESCE(VALORCREDITO,0)), SUM(COALESCE(VALORDEBITO,0)) from MOVIMENTOCONT where data < :DTAINI 
  ...
  DO BEGIN
    ...
    IF ((DESCRICAO IS NULL) or (DESCRICAO = '')) THEN
      ...
      DESCRICAO = UDF_LEFT(:FORN, 50) || ' - ' || UDF_LEFT(:DESCRICAO,150) || '-NF: ' ||  :N_TITULO;
    ORDEM = 1;
  ...
  DO BEGIN
    ...
    IF ((DESCRICAO IS NULL) or (DESCRICAO = '')) THEN
      ...
      DESCRICAO = UDF_LEFT(:FORN, 50) || ' - ' || UDF_LEFT(:DESCRICAO,150)|| '-' || UDF_LEFT(:CONTACONTABIL,50) || '-NF: ' || :N_TITULO;

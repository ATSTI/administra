...
BEGIN
  ...
  /* ########### Gerando a Despesas ############*/
  FOR select sum((md.QUANTIDADE*md.VLR_BASE)+md.VIPI+md.FRETE+md.ICMS_SUBST)
     FROM MOVIMENTO mov 
       inner join MOVIMENTODETALHE md on md.CODMOVIMENTO = mov.CODMOVIMENTO
       inner join COMPRA comp on comp.CODMOVIMENTO = mov.CODMOVIMENTO     
       inner join PRODUTOS prod on prod.CODPRODUTO = md.CODPRODUTO 
     where (mov.CODNATUREZA = 4) 
       and (comp.DATACOMPRA between :PDTA1 AND :PDTA2)
       and ((comp.CODCCUSTO = :PCC) OR (:PCC = 0))
  INTO :CUSTO
  do begin   
  ...
  /* Total dos produtos COMPRADOS por Centro de Custo  */
  /* Total dos produtos COMPRADOS por Centro de Custo  */
  FOR select distinct prod.FAMILIA, sum((md.QUANTIDADE*md.VLR_BASE)+md.VIPI+md.FRETE+md.ICMS_SUBST) 
     FROM MOVIMENTO mov 
       inner join MOVIMENTODETALHE md on md.CODMOVIMENTO = mov.CODMOVIMENTO
       inner join COMPRA comp on comp.CODMOVIMENTO = mov.CODMOVIMENTO     
       inner join PRODUTOS prod on prod.CODPRODUTO = md.CODPRODUTO 
    ...
    do begin 
      ...
      FOR select distinct prod.CATEGORIA,  sum((md.QUANTIDADE*md.VLR_BASE)+md.VIPI+md.FRETE+md.ICMS_SUBST)
       FROM MOVIMENTO mov 
       inner join MOVIMENTODETALHE md on md.CODMOVIMENTO = mov.CODMOVIMENTO
       inner join COMPRA comp on comp.CODMOVIMENTO = mov.CODMOVIMENTO     
       inner join PRODUTOS prod on prod.CODPRODUTO = md.CODPRODUTO 
      ...
      do begin
        ...
        FOR select (prod.CODPRO || md.DESCPRODUTO) AS PRODUTO, sum((md.QUANTIDADE*md.VLR_BASE)+md.VIPI+md.FRETE+md.ICMS_SUBST)
            FROM MOVIMENTO mov 
           inner join MOVIMENTODETALHE md on md.CODMOVIMENTO = mov.CODMOVIMENTO
           inner join COMPRA comp on comp.CODMOVIMENTO = mov.CODMOVIMENTO     
           inner join PRODUTOS prod on prod.CODPRODUTO = md.CODPRODUTO 
           where (mov.CODNATUREZA = 4) 
             and (comp.DATACOMPRA between :PDTA1 AND :PDTA2)
             and ((comp.CODCCUSTO = :PCC) OR (:PCC = 0))
             and (prod.CATEGORIA = :linha)
           group by prod.CODPRO, md.DESCPRODUTO
        INTO :PRODUT, :CUSTO
        do begin    
          IF (CUSTO IS NULL) THEN
            CUSTO = 0;
          credito = custo;       
          DESC_CONTA = '       ' || UDF_TRIM(PRODUT);

          SUSPEND; 
        end
      end -- CATEGORIAPRODUTO
    end -- FAMILIAPRODUTOS

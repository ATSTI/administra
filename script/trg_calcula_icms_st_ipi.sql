...
BEGIN
  ...
  -- versao 4.5.0.0  12/09/2017 - calculo ii 
  new.SUITE = ''; 
  new.Aliq_CUPOM = 'II';
  ...
  begin 
    ...
    begin
			...
			if (vd is null) then 
				...
				begin
					...
					if (new.DESCONTO_BC = 'True') then
						new.VLR_BASEICMS = UDF_ROUNDDEC(( (TOTALITENS + new.VIPI + :vFrete - :vd)* ind_reduzicms), :arredondar);
					
					-- Soma Frete na BC ICMS e Nao Subtrai Desconto da BC ICMS
					else	
						new.VLR_BASEICMS = UDF_ROUNDDEC(( ((TOTALITENS + new.VIPI + :vFrete))* ind_reduzicms), :arredondar);
				end
				else
				begin
					-- Subtrair Desconto a BC ICMS 
					if (new.DESCONTO_BC <> 'True') then
						new.VLR_BASEICMS = UDF_ROUNDDEC(( (TOTALITENS + new.VIPI - :vd )* ind_reduzicms), :arredondar);
					-- Nao Subtrai Desconto da BC ICMS, nao soma Frete a BC ICMS
					else
					  new.VLR_BASEICMS = UDF_ROUNDDEC(( TOTALITENS * ind_reduzicms) + new.VIPI, :arredondar);
				end
		...
		---------------------------------------------------------------------------------
        -- new.DESCPRODUTO = new.CFOP || '-' || :UF || '-' || :PESSOA;
        if (new.SUITE = '') then 
		begin
            select COALESCE(cfn.ICMS_SUBST, 0), COALESCE(cfn.ICMS_SUBST_IC, 0), COALESCE(cfn.ICMS_SUBST_IND, 0), COALESCE(cfn.ICMS, 0), 
            COALESCE(cfn.ICMS_BASE, 1)
	     ...
	     , COALESCE(cfn.II_PERCENTUAL,0)
            ...
            , :CST_IPI_CENQ, :aliq_est, :aliq_mun, :redBasePIS, :redBaseCOFINS, :redBaseIPI, :II_PERCENTUAL;
            ...
            begin
              new.SUITE = 'Trib. NCM';
                ...
                begin 
                  ...
                  new.II = UDF_ROUNDDEC((new.BCII) * (:II_PERCENTUAL / 100), :arredondar);
                  new.VLRBC_IPI = UDF_ROUNDDEC((TOTALITENS+new.II) * redBaseIPI , :arredondar);
                  new.VIPI = UDF_ROUNDDEC(((((new.VLR_BASE*new.QUANTIDADE)++new.II)*redBaseIPI) * IND_IPI/100), :arredondar);
                  new.PIPI = IND_IPI;
                  ...
                  cicms = 1 - (:cicms / 100);
                  --new.icms = :cicms;
                  new.VLR_BASEICMS = UDF_ROUNDDEC((new.VLR_BASEICMS / :cicms), :arredondar);
		      ...
		      COALESCE(vFCPUFDest,0), COALESCE(vICMSUFDest,0), COALESCE(vICMSUFRemet,0), COALESCE(CST_IPI_CENQ, '999')
                ...
                begin
                    ...
                    begin
                        --Subtrair Desconto a BC ICMS 
                        if (new.DESCONTO_BC <> 'True') then
                            new.VLR_BASEICMS = UDF_ROUNDDEC(( (TOTALITENS - :vd )* ind_reduzicms) + new.VIPI, :arredondar);
                        --Nao Subtrai Desconto da BC ICMS, nao soma Frete a BC ICMS
                        else
                            new.VLR_BASEICMS = UDF_ROUNDDEC(( TOTALITENS * ind_reduzicms) + new.VIPI, :arredondar);
                    end

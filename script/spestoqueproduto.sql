...
BEGIN
    FOR SELECT CODPRODUTO, CODPRO, PRODUTO, FAMILIA, CATEGORIA,  VALORUNITARIOATUAL, VALOR_PRAZO, TIPO
      ,PRECOMEDIO, tipoPrecoVenda, margem FROM PRODUTOS 
        WHERE CODPRODUTO BETWEEN :PROD1 AND :PROD2  AND ((tipo <> 'SERV') or (TIPO is null)) AND ((CATEGORIA = :SUBGRUPO) OR 
                 ...
                 AND ((MARCA = :MARCA) OR (:MARCA = 'TODAS AS MARCAS CADASTRADAS'))
                 AND (USA IS NULL) ORDER BY PRODUTO
  ...
  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
       ...
       -- SALDOS ANTERIORES DE ENTRADA E SAIDA 04/12/2015
      SELECT (sum(EM.QTDECOMPRA)+sum(em.QTDEENTRADA)) TOTALENTRADA, (sum(EM.QTDESAIDA)+sum(em.QTDEVENDA)) FROM ESTOQUEMES EM 
       WHERE em.MESANO < :DTA1 
         AND em.CODPRODUTO = :COD
         AND ((em.CENTROCUSTO = :PCCUSTO) OR (:PCCUSTO = 0))
...
GRANT EXECUTE
 ON PROCEDURE SPESTOQUEPRODUTO TO  SYSDBA;

...
BEGIN
  ...
  SELECT SUM(rec.VALORRECEBIDO + rec.JUROS) FROM RECEBIMENTO rec 
    ...
    and ((rec.CONTADEBITO  = :CCUSTO) or (:CCUSTO = 0))       
  ...
  BEGIN
    ...
    FOR select mov.DATA, SUM(mov.VALORDEBITO), his.HISTORICO, pc.CODREDUZIDO, mov.FORMA from MOVIMENTOCONT mov
       ...
       WHERE pc.CONTA = mov.CONTA and mov.DATA  = :DATAFAZ
       and mov.tipoorigem = 'CONTABIL' 
       and mov.CONTA = :NCONTA 
  ...
  BEGIN 
    SELECT CONTA, NOME FROM PLANO WHERE CODIGO = :COD_CAIXA
      INTO :NCONTA, :CAIXA;
       FOR select mov.DATA, SUM(mov.VALORDEBITO), his.HISTORICO, pc.CODREDUZIDO, mov.FORMA, PC.NOME
             from MOVIMENTOCONT mov
      INNER JOIN  PLANO pc on pc.CONTA = mov.CONTA    
       LEFT OUTER JOIN HISTORICO_CONTAB his on his.COD_CONTAB = mov.CODORIGEM 
      WHERE mov.DATA  = :DATAFAZ
        and mov.tipoorigem = 'CONTABIL' 
        and PLNCTAMAIN(mov.CONTA) = PLNCTAMAIN(:CONTACAIXA) 
        and ((MOV.CODCCUSTO = :CCUSTO) or (:CCUSTO = 0)) 
        and ((mov.FORMA = :FORMAPAG) or (:FORMAPAG = 'T'))
      group by mov.DATA, his.HISTORICO, pc.CODREDUZIDO, mov.FORMA, PC.NOME
       INTO :DTAPAGTO,:VALORD, :DESCRICAO, :CODCONTA, :FORMA, :CONTACONTABIL 
    do begin
      VALOR = VALOR + VALORD;
      ORDEM = ORDEM + 1;
      DESCRICAO = DESCRICAO || ORDEM ||  'xxx' ;
      --if (DESCRICAO = 'ABERTURA DE CAIXA') then
      --  ORDEM = 0;
      N_T = 'CONTABIL';  
      IF (VALORD > 0.001) THEN
  ...
  FOR SELECT rec.DATARECEBIMENTO, CAST(rec.CODCLIENTE AS VARCHAR(5)) || '-' ||  cli.NOMECLIENTE, 
    ...
    and rec.DATARECEBIMENTO = :DATAFAZ
    and ((rec.CAIXA = :COD_CAIXA) or (:COD_CAIXA = 0))
    and ((rec.CONTADEBITO = :CCUSTO) or (:CCUSTO = 0))       
    and ((rec.FORMARECEBIMENTO = :FORMAPAG) or (:FORMAPAG = 'T'))
    and (rec.STATUS = '7-')
  ...
  BEGIN
    SELECT CONTA, NOME FROM PLANO WHERE CODIGO = :COD_CAIXA
      INTO :NCONTA, :CAIXA;
    FOR select mov.DATA, SUM(mov.VALORCREDITO), his.HISTORICO, pc.CODREDUZIDO, mov.FORMA 
          from MOVIMENTOCONT mov
    INNER JOIN PLANO pc on pc.CONTA = mov.CONTA    
    LEFT OUTER JOIN HISTORICO_CONTAB his on his.COD_CONTAB = mov.CODORIGEM  
         WHERE mov.DATA  = :DATAFAZ

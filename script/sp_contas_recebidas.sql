...
BEGIN
  ...
  FOR SELECT rec.DATAVENCIMENTO, rec.DATARECEBIMENTO, rec.CODCLIENTE, CAST(rec.CODCLIENTE AS VARCHAR(10)) || '-' ||  cli.NOMECLIENTE,
    rec.HISTORICO, rec.VALOR_RESTO, rec.VALORRECEBIDO + rec.JUROS, rec.CONTACREDITO,rec.CAIXA, rec.TITULO
    FROM RECEBIMENTO rec, CLIENTES  cli where cli.CODCLIENTE = rec.CODCLIENTE
    and rec.DATARECEBIMENTO BETWEEN :DTAINI AND :DTAFIM  AND ((rec.CODALMOXARIFADO = :CCUSTO) or (:CCUSTO = 0))

...
DECLARE VARIABLE tEntradaEstoque double precision;
DECLARE VARIABLE vEntradaEstoque double precision;
DECLARE VARIABLE tFinalEstoque double precision;
DECLARE VARIABLE vFinalEstoque double precision;
...
BEGIN
  ...
  do begin 
    ...
    --CUSTO ITEM 
    -- estoque inicial
    SELECT FIRST 1 ev.PRECOCUSTO, ev.SALDOFIMACUM FROM ESTOQUE_VIEW_CUSTO (udf_incday(:pdta1,-1),
      :CodPro,  :CCUSTO, 'TODOS OS LOTES CADASTRADOS NO SISTEMA') ev
     into :vlrCustoTotal, :qtdeEstoque; 
         
    -- qtde e preco do que entrou   
    ...
    into :vEntradaEstoque, :tEntradaEstoque;  

     if (vEntradaEstoque is null) then 
       vEntradaEstoque = 0;
     
     if (tEntradaEstoque is null) then 
       tEntradaEstoque = 0;

     
    -- estoque final     
    SELECT FIRST 1 ev.PRECOCUSTO, ev.SALDOFIMACUM FROM ESTOQUE_VIEW_CUSTO (:pdta2,
      :CodPro,  :CCUSTO, 'TODOS OS LOTES CADASTRADOS NO SISTEMA') ev
     into :vFinalEstoque, :tFinalEstoque; 

     if (vFinalEstoque is null) then 
       vFinalEstoque = 0;

     if (tFinalEstoque is null) then 
       tFinalEstoque = 0;
    ...
    if ((tEntradaEstoque+qtdeEstoque-tFinalEstoque)>0) then 
      vlrCustoTotal = (((vlrCustoTotal*qtdeEstoque)+(vEntradaEstoque*tEntradaEstoque)-(vFinalEstoque*tFinalEstoque)))/(tEntradaEstoque+qtdeEstoque-tFinalEstoque);
    else
      vlrCustoTotal = 0;

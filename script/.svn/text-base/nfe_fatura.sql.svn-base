...
BEGIN
  ...
  select first 1 ec.UF from ENDERECOCLIENTE ec where ec.CODCLIENTE = :cli and ec.TIPOEND = 0
    ...
    if (:nenv = 'N') then
    begin
		for select r.DATAVENCIMENTO, r.via, r.VALOR_RESTO, parcelas from RECEBIMENTO r  where r.CODVENDA = :codv order by r.DATAVENCIMENTO, r.VIA
		into :datafatura, :numerofatura, :valor, :parc
		do begin
			--parc = parc + 1;

        ...
        for invoice in inv_obj.browse(cr, uid, active_ids, context=context):
            ...
            if invoice.state in ['proforma2', 'open', 'sefaz_export']:
                ...
                for move_line in self.pool.get('account.move.line').browse(cr, uid, acc_line):
                    ...
                    'cedente': company.id,
                    #'cedente_cidade': company.l10n_br_city_id.name, 
                    #'cedente_uf': company.state_id.code,
                    #'cedente_logradouro': company.street,
                    #'cedente_bairro': company.district,
                    #'cedente_cep': company.zip,
                    #'cedente_cnpj_cpf': company.partner_id.cnpj_cpf,
                    'sacado': partner.id,
...
#                    'multa': partner.boleto_partner_config.multa,
                    ...
                    #if bol_conf.banco == 'itau':
                    #    boleto['codigo_banco'] = 184

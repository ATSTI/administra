...
begin
  ...
  nao_testa_data := '';
  nfe_serie_receita := 1;
  // SAT - N�mero de S�rie do equipamento SAT
...
begin
  ...
  begin
    Case dia of
      1 : mensagemInicial := 'A Licen�a de uso expirar� em 9 dias.';
      2 : mensagemInicial := 'A Licen�a de uso expirar� em 8 dias.';
      3 : mensagemInicial := 'A Licen�a de uso expirar� em 7 dias.';
      4 : mensagemInicial := 'A Licen�a de uso expirar� em 6 dias.';
      5 : mensagemInicial := 'A Licen�a de uso expirar� em 5 dias.';
      6 : mensagemInicial := 'A Licen�a de uso expirar� em 4 dias.';
      7 : mensagemInicial := 'A Licen�a de uso expirar� em 3 dias.';
      8 : mensagemInicial := 'A Licen�a de uso expirar� em 2 dias.';
      9 : mensagemInicial := 'A Licen�a de uso expirar� em 1 dia.';
      10 : mensagemInicial := 'A Licen�a de uso expirar� hoje.';
      11 : begin
             mensagemInicial := 'A Licen�a de uso expirada.';
             sistemaLiberado := 'N';
           end;
  ...
  begin
    Case dia of
      11 : mensagemInicial := 'A Licen�a de uso expirar� em 9 dias.';
      12 : mensagemInicial := 'A Licen�a de uso expirar� em 8 dias.';
      13 : mensagemInicial := 'A Licen�a de uso expirar� em 7 dias.';
      14 : mensagemInicial := 'A Licen�a de uso expirar� em 6 dias.';
      15 : mensagemInicial := 'A Licen�a de uso expirar� em 5 dias.';
      16 : mensagemInicial := 'A Licen�a de uso expirar� em 4 dias.';
      17 : mensagemInicial := 'A Licen�a de uso expirar� em 3 dias.';
      18 : mensagemInicial := 'A Licen�a de uso expirar� em 2 dias.';
      19 : mensagemInicial := 'A Licen�a de uso expirar� em 1 dia.';
      20 : mensagemInicial := 'A Licen�a de uso expira hoje.';
      21 : begin
             mensagemInicial := 'A Licen�a de uso expirada.';
             sistemaLiberado := 'N';
           end;
...
begin
  ...
  begin
    MessageDlg('Este centro de Resultado est� fechado para movimenta��o nesta data.', mtWarning, [mbOK], 0);
    Result := True;
  end;
end;
...
begin
  ...
  begin
    ...
    if (not sqlBusca.IsEmpty) then
      ...
      ' n�o est� correto, contacte a ATS.', mtError, [mbOK], 0);
    ...
    sqlBusca.Open;
    if (not sqlBusca.IsEmpty) then
      MessageDlg('Tamanho do campo OBSERVA��O NA VENDA ' + #13#10 +
      ' n�o est� correto, contacte a ATS.', mtError, [mbOK], 0);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      dadosClienteFaltando := 'UF n�o informado;'+#13#10;
    end;
    if (sqlBusca.fieldByName('RAZAOSOCIAL').AsString = '') then
    begin
      dadosClienteFaltando := dadosClienteFaltando + 'RAZAO SOCIAL n�o informada;'+#13#10 ;
    end;
    if (sqlBusca.fieldByName('PAIS').AsString = '') then
    begin
      dadosClienteFaltando := dadosClienteFaltando + 'PA�S n�o informado;'+#13#10;
    end;
    if (sqlBusca.fieldByName('CNPJ').AsString = '') then
    begin
      if (sqlBusca.fieldByName('UF').AsString <> 'EX') then
        dadosClienteFaltando := dadosClienteFaltando + 'CNPJ/CPF n�o informado;'+#13#10;
    end;
    {if (scds_cliente_procINSCESTADUAL.AsString = '') then
    begin
      if (scds_cliente_procUF.AsString <> 'EX') then
        dadosClienteFaltando := dadosClienteFaltando + 'INSC. ESTADUAL/RG n�o informada;'+#13#10;
    end;}
    if (sqlBusca.fieldByName('LOGRADOURO').AsString = '') then
    begin
      if (sqlBusca.fieldByName('UF').AsString <> 'EX') then
        dadosClienteFaltando := dadosClienteFaltando + 'ENDERE�O n�o informado;'+#13#10;
    end;
    if (sqlBusca.fieldByName('BAIRRO').AsString = '') then
    begin
      if (sqlBusca.fieldByName('UF').AsString <> 'EX') then
        dadosClienteFaltando := dadosClienteFaltando + 'BAIRRO n�o informado;'+#13#10;
    end;
    if (sqlBusca.fieldByName('CIDADE').AsString = '') then
    begin
      if (sqlBusca.fieldByName('UF').AsString <> 'EX') then
        dadosClienteFaltando := dadosClienteFaltando + 'CIDADE n�o informada;'+#13#10;
    end;
    if (sqlBusca.fieldByName('CEP').AsString = '') then
    begin
      if (sqlBusca.fieldByName('UF').AsString <> 'EX') then
        dadosClienteFaltando := dadosClienteFaltando + 'CEP n�o informado;'+#13#10;
    end;
    if (sqlBusca.fieldByName('CD_IBGE').AsString = '') then
    begin
      if (sqlBusca.fieldByName('UF').AsString <> 'EX') then
        dadosClienteFaltando := dadosClienteFaltando + 'C�D. IBGE n�o informado;'+#13#10;
    end;
  end;
  if (dadosClienteFaltando <> '') then
  begin
    MessageDlg('Falta informa��o no cadastro do cliente, para criar uma nota fiscal :' + #13#10 +
...
begin
  ...
  begin
    result := 'Cliente com cadastro "BLOQUEADO",  venda n�o permitida.';
    exit
  end;}

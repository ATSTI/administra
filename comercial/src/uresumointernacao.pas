...
uses
  ...
  Provider, DBClient, SqlExpr, Mask, Buttons, DBCtrls, rpcompobase,
  rpvclreport, JvExMask, JvToolEdit, JvMaskEdit, JvCheckedMaskEdit,
  JvDatePickerEdit, JvBaseEdits;

type
  Tfresumointernacao = class(TForm)
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    Label4: TLabel;
...
var
  fresumointernacao: Tfresumointernacao;
...
procedure Tfresumointernacao.BitBtn3Click(Sender: TObject);
begin
...
procedure Tfresumointernacao.BitBtn1Click(Sender: TObject);
...
begin
 ...
 Edit7.Text := IntToStr(msg.PrestadorParaOperadora.LoteGuias.NumeroLote);
  ...
  j := msg.PrestadorParaOperadora.LoteGuias.Guias.GuiaFaturamento.GuiaResumoInternacao.Count;

  if (i <= (j-1)) then
  begin

    With (msg.PrestadorParaOperadora.LoteGuias.Guias.GuiaFaturamento.GuiaResumoInternacao.Items[i]) do
    begin

      label52.Caption := DadosBeneficiario.NomeBeneficiario;


      Edit8.Text := IdentificacaoGuiaInternacao.RegistroANS;
     ...
     // MaskEdit3.Text := IdentificacaoGuiaInternacao.DataEmissaoGuia;
      Edit58.Text := IdentificacaoGuiaInternacao.DataEmissaoGuia;
      Edit10.Text := IdentificacaoGuiaInternacao.NumeroGuiaPrestador;
      Edit11.Text := IdentificacaoGuiaInternacao.NumeroGuiaOperadora;
      ...
      Edit29.Text := IdentificacaoExecutante.NumeroCNES;
      Edit16.Text := IdentificacaoExecutante.NomeContratado;
      MaskEdit1.Text := IdentificacaoExecutante.Identificacao.CNPJ;

      MaskEdit6.Text := IdentificacaoExecutante.Identificacao.CPF;
{
   ...
   ///   Edit17.Text :=
 {
    ...
    //  Edit23.Text := DiagnosticosSaidaInternacao.DiagnosticoPrincipal.CodigoDiagnostico;
{
      Edit39.Text := DiagnosticoAtendimento.TipoDoenca;

      Edit40.Text := DiagnosticoAtendimento.TempoReferidoEvolucaoDoenca.UnidadeTempo;
}
   //   Edit40.Text := DiagnosticosSaidaInternacao.DiagnosticoPrincipal.CodigoDiagnostico..TempoReferidoEvolucaoDoenca.UnidadeTempo;

      Edit59.Text := DiagnosticosSaidaInternacao.DiagnosticoPrincipal.DescricaoDiagnostico;
 ...
 //////////////////
      ...
      }
    {
     ...
     // Edit49.Text := codigoDiagnostico ;
      ...
      Edit24.Text := DataHoraInternacao;
...
//      Edit27.Text :=  TipoConsulta;
 ...
 //////     Edit30.Text := TipoSaida;
  ...
  for k := 0 to j-1  do
  begin
    With (msg.PrestadorParaOperadora.LoteGuias.Guias.GuiaFaturamento.GuiaResumoInternacao.Items[k]) do
    begin
  ...
  if (j = 0 ) then
   MessageDlg(' Esse Arquivo não é o um " Resumo Internação" ', mtError, [mbOK], 0);
  exit;

  
end;

procedure Tfresumointernacao.btnAnteriorClick(Sender: TObject);
begin
  i :=  i - 1;
  if (i < 0) then
    i :=  0;
...
procedure Tfresumointernacao.btnProximoClick(Sender: TObject);
begin
  i :=  i + 1;
  if (i > j) then
    i := j;
...
procedure Tfresumointernacao.BitBtn5Click(Sender: TObject);
Var msg: I213_MensagemTISS;
    I : integer ;
begin
  ...
  msg := LoadmensagemTISS(Edit28.Text);
  j := msg.PrestadorParaOperadora.LoteGuias.Guias.GuiaFaturamento.GuiaResumoInternacao.Count; ///msg.PrestadorParaOperadora.LoteGuias.Guias.GuiaFaturamento.GuiaSP_SADT.Count;
  for I := 0 to j - 1 do
  begin
  ...
  if(msg.PrestadorParaOperadora.LoteGuias.NumeroLote = cdsLocateNUMEROLOTE.AsInteger) then
    ...
    begin
      ...
      cdsxmlTIPO_GUIA.AsInteger := 2 ; // 1 PARA FATURAMENTO  2 PARA RESUMO INTERNAÇÃO
      ...
      cdsxmlNUMERO_GUIA_PRESTADOR.AsString := IdentificacaoGuiaInternacao.NumeroGuiaPrestador;

      cdsxmlDATA_IMPORTACAO.AsDateTime := StrToDate(DATA_IMPORTA.Text) ;



      cdsxmlDATA_EMISSAO.AsString := IdentificacaoGuiaInternacao.DataEmissaoGuia;

      cdsxmlNOME_CONTRATADO.AsString := IdentificacaoExecutante.NomeContratado;
...
procedure Tfresumointernacao.BitBtn2Click(Sender: TObject);
begin
  VCLReport1.Filename := str_relatorio + 'resumointernacao.rep';
  VCLReport1.Report.DatabaseInfo.Items[0].SQLConnection := dm.sqlsisAdimin;
  VCLReport1.Report.Params.ParamByName('LOTE').Value := DBComboBox1.Text;
  VCLReport1.Execute;
end;

procedure Tfresumointernacao.FormShow(Sender: TObject);
begin
  if (DMSaude.cdslote.Active) then
    DMSaude.cdslote.Close;
    DMSaude.cdslote.Params[0].AsInteger := 2 ;  

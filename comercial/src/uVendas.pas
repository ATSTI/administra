...
type
  ...
  public
    ...
    usalote, matPrima, inseridoMatPrima, vendaexiste, usaprecolistavenda, CODIGOPRODUTO, margemVenda, estoque_negativo : string; //, tipoVenda
...
begin
  ...
  {------Pesquisando na tab Parametro Centro de Receita Padrão ---------}
    ...
    if cds_ccusto.Active then
      cds_ccusto.Close;
    cds_ccusto.Params[0].AsString := dm.dmCentroReceita;
    cds_ccusto.Open;
    // populo a combobox
    cds_ccusto.First;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        texto6 := texto6 + ' ' + Copy(produto_cupom, 0, StrToInt(dm.linhaTamanho));       //descrição do produto
        Writeln(Impressora, c17cpi, RemoveAcento(texto6));
        if (length(produto_cupom)>(StrToInt(dm.linhaTamanho)*1)) then
        begin
          texto6 := '    ' + Copy(produto_cupom, (StrToInt(dm.linhaTamanho)*1)+1, StrToInt(dm.linhaTamanho));       //descrição do produto
          Writeln(Impressora, c17cpi, RemoveAcento(texto6));
        end;
        if (length(produto_cupom)>(StrToInt(dm.linhaTamanho)*2)) then
        begin
          texto6 := '    ' + Copy(produto_cupom, (StrToInt(dm.linhaTamanho)*2)+1, StrToInt(dm.linhaTamanho));       //descrição do produto
          Writeln(Impressora, c17cpi, RemoveAcento(texto6));
        end;
        if (length(produto_cupom)>(StrToInt(dm.linhaTamanho)*3)) then
        begin
          texto6 := '    ' + Copy(produto_cupom, (StrToInt(dm.linhaTamanho)*3)+1, StrToInt(dm.linhaTamanho));       //descrição do produto
          Writeln(Impressora, c17cpi, RemoveAcento(texto6));
        end;
        if (length(produto_cupom)>(StrToInt(dm.linhaTamanho)*4)) then
        begin
          texto6 := '    ' + Copy(produto_cupom, (StrToInt(dm.linhaTamanho)*4)+1, StrToInt(dm.linhaTamanho));       //descrição do produto
          Writeln(Impressora, c17cpi, RemoveAcento(texto6));
        end;
        if (length(produto_cupom)>(StrToInt(dm.linhaTamanho)*5)) then
        begin
          texto6 := '    ' + Copy(produto_cupom, (StrToInt(dm.linhaTamanho)*5)+1, StrToInt(dm.linhaTamanho));       //descrição do produto
          Writeln(Impressora, c17cpi, RemoveAcento(texto6));
        end;
        if (length(produto_cupom)>(StrToInt(dm.linhaTamanho)*6)) then
        begin
          texto6 := '    ' + Copy(produto_cupom, (StrToInt(dm.linhaTamanho)*6)+1, StrToInt(dm.linhaTamanho));       //descrição do produto
          Writeln(Impressora, c17cpi, RemoveAcento(texto6));
        end;
        if (length(produto_cupom)>(StrToInt(dm.linhaTamanho)*7)) then
        begin
          texto6 := '    ' + Copy(produto_cupom, (StrToInt(dm.linhaTamanho)*7)+1, StrToInt(dm.linhaTamanho));       //descrição do produto
          Writeln(Impressora, c17cpi, RemoveAcento(texto6));
        end;
...
begin
  ...
  if (not cds_movimento.isempty) then
    label15.Caption := 'Pedido - ' + IntToStr(cds_MovimentoCODMOVIMENTO.asInteger);  
end;

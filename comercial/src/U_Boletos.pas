   ...
   ACBrBoleto1.Cedente.CNPJCPF := s_empresaCNPJ_CPF.AsString;
      ...
      begin
           ...
           begin
              ...
              begin
                ...
                begin
                  ...
                  case StrToInt(s_bancoPROTESTO.AsString) of
                    ...
                    01: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,1); //01 - Cobrar juros (Dispens·vel se informado o valor a ser cobrado por dia de atraso).
                    03: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,3);//03 - Protestar no 3∫ dia ˙til apÛs vencido
                    04: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,4);//04 - Protestar no 4∫ dia ˙til apÛs vencido
                    05: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,5);//05 - Protestar no 5∫ dia ˙til apÛs vencido
                    ...
                    10: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,10);//10 - Protestar no 10∫ dia corrido apÛs vencido
                    15: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,15);//15 - Protestar no 15∫ dia corrido apÛs vencido
                    20: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,20);//20 - Protestar no 20∫ dia corrido apÛs vencido
                    //22: TItulo.DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,21)//22 - Conceder desconto sÛ atÈ a data estipulada
                    25: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,25);//25 - Protestar no 25∫ dia corrido apÛs vencido
                    30: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,30);//30 - Protestar no 30∫ dia corrido apÛs vencido
                    45: DataProtesto :=  IncDay(ds_crDATAVENCIMENTO.AsDateTime,45);//45 - Protestar no 45∫ dia corrido apÛs vencido
                  else
              ...
              begin
                LocalPagamento :=  'AtÈ o vencimento, preferencialmente no Ita˙ e ApÛs o vencimento, somente no Ita˙';
              end;

object fCarne: TfCarne
  Left = 111
  Top = 30
  Width = 952
  Height = 850
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object BoletoCarne: TRLReport
    Left = 24
    Top = 2
    Width = 794
    Height = 1172
    DataSource = ds_Cr
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    Margins.LeftMargin = 8.000000000000000000
    Margins.TopMargin = 5.000000000000000000
    Margins.RightMargin = 5.000000000000000000
    Margins.BottomMargin = 5.000000000000000000
    PageSetup.PaperSize = fpCustom
    PageSetup.PaperWidth = 210.000000000000000000
    PageSetup.PaperHeight = 310.000000000000000000
    PreviewOptions.ShowModal = True
    object RLBand3: TRLBand
      Left = 30
      Top = 19
      Width = 745
      Height = 367
      BeforePrint = RLBand3BeforePrint
      object RLDraw30: TRLDraw
        Left = 150
        Top = 286
        Width = 599
        Height = 17
        DrawKind = dkLine
      end
      object RLDraw31: TRLDraw
        Left = 2
        Top = 240
        Width = 754
        Height = 1
        DrawKind = dkLine
      end
      object RLDraw32: TRLDraw
        Left = 567
        Top = 211
        Width = 220
        Height = 1
        DrawKind = dkLine
      end
      object RLDraw51: TRLDraw
        Left = 567
        Top = 184
        Width = 220
        Height = 1
        DrawKind = dkLine
      end
      object RLDraw52: TRLDraw
        Left = -3
        Top = 157
        Width = 759
        Height = 1
        DrawKind = dkLine
      end
      object RLDraw53: TRLDraw
        Left = 296
        Top = 132
        Width = 1
        Height = 26
        Angle = 90.000000000000000000
        DrawKind = dkLine
      end
      object RLDraw54: TRLDraw
        Left = -1
        Top = 132
        Width = 757
        Height = 1
        DrawKind = dkLine
      end
      object RLDraw55: TRLDraw
        Left = 476
        Top = 106
        Width = 1
        Height = 52
        Angle = 90.000000000000000000
        DrawKind = dkLine
      end
      object RLDraw56: TRLDraw
        Left = 415
        Top = 106
        Width = 1
        Height = 27
        Angle = 90.000000000000000000
        DrawKind = dkLine
      end
      object RLDraw57: TRLDraw
        Left = 344
        Top = 106
        Width = 1
        Height = 52
        Angle = 90.000000000000000000
        DrawKind = dkLine
      end
      object RLDraw58: TRLDraw
        Left = 245
        Top = 106
        Width = 1
        Height = 52
        Angle = 90.000000000000000000
        DrawKind = dkLine
      end
      object RLDraw59: TRLDraw
        Left = -1
        Top = 106
        Width = 757
        Height = 1
        DrawKind = dkLine
      end
      object RLDraw60: TRLDraw
        Left = 0
        Top = 46
        Width = 756
        Height = 16
        DrawKind = dkLine
      end
      object RLDraw61: TRLDraw
        Left = -1
        Top = 82
        Width = 757
        Height = 1
        DrawKind = dkLine
      end
      object imgBancoCarne: TRLImage
        Left = 152
        Top = 14
        Width = 101
        Height = 37
        Center = True
        Scaled = True
      end
      object RLDraw63: TRLDraw
        Left = 253
        Top = 13
        Width = 1
        Height = 41
        Angle = 90.000000000000000000
        DrawKind = dkLine
      end
      object RLLabel1: TRLLabel
        Left = 154
        Top = 55
        Width = 70
        Height = 10
        Caption = 'Local de Pagamento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel94: TRLLabel
        Left = 572
        Top = 55
        Width = 41
        Height = 10
        Caption = 'Vencimento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLDraw65: TRLDraw
        Left = 567
        Top = 53
        Width = 1
        Height = 188
        Angle = 90.000000000000000000
        DrawKind = dkLine
      end
      object RLLabel95: TRLLabel
        Left = 154
        Top = 83
        Width = 30
        Height = 10
        Caption = 'Cedente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel97: TRLLabel
        Left = 572
        Top = 83
        Width = 91
        Height = 10
        Caption = 'Ag'#234'ncia / C'#243'digo Cedente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel99: TRLLabel
        Left = 152
        Top = 108
        Width = 68
        Height = 10
        Caption = 'Data do Documento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel101: TRLLabel
        Left = 247
        Top = 108
        Width = 79
        Height = 10
        Caption = 'N'#250'mero do Documento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel103: TRLLabel
        Left = 347
        Top = 108
        Width = 47
        Height = 10
        Caption = 'Esp'#233'cie Doc.'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel105: TRLLabel
        Left = 418
        Top = 108
        Width = 23
        Height = 10
        Caption = 'Aceite'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel107: TRLLabel
        Left = 479
        Top = 108
        Width = 82
        Height = 10
        Caption = 'Data do Processamento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel109: TRLLabel
        Left = 572
        Top = 108
        Width = 53
        Height = 10
        Caption = 'Nosso N'#250'mero'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel111: TRLLabel
        Left = 152
        Top = 133
        Width = 49
        Height = 10
        Caption = 'Uso do Banco'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel113: TRLLabel
        Left = 247
        Top = 133
        Width = 30
        Height = 10
        Caption = 'Carteira'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel115: TRLLabel
        Left = 298
        Top = 133
        Width = 29
        Height = 10
        Caption = 'Esp'#233'cie'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel5: TRLLabel
        Left = 298
        Top = 143
        Width = 44
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Caption = 'R$'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel117: TRLLabel
        Left = 348
        Top = 133
        Width = 40
        Height = 10
        Caption = 'Quantidade'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel119: TRLLabel
        Left = 479
        Top = 133
        Width = 20
        Height = 10
        Caption = 'Valor'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel121: TRLLabel
        Left = 572
        Top = 133
        Width = 87
        Height = 10
        Caption = '( = ) Valor do Documento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel123: TRLLabel
        Left = 152
        Top = 158
        Width = 325
        Height = 10
        Caption = 
          'Instru'#231#245'es (Todas as informa'#231#245'es deste bloqueto s'#227'o de exclusiva' +
          ' responsabilidade do cedente.)'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel124: TRLLabel
        Left = 572
        Top = 158
        Width = 92
        Height = 10
        Caption = '( - ) Desconto / Abatimento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel126: TRLLabel
        Left = 572
        Top = 185
        Width = 62
        Height = 10
        Caption = '( + ) Mora / Multa'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel128: TRLLabel
        Left = 572
        Top = 212
        Width = 68
        Height = 10
        Caption = '( = ) Valor Cobrado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel130: TRLLabel
        Left = 152
        Top = 241
        Width = 27
        Height = 10
        Caption = 'Sacado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel132: TRLLabel
        Left = 571
        Top = 241
        Width = 82
        Height = 10
        Caption = 'CPF / CNPJ do Sacado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel135: TRLLabel
        Left = 571
        Top = 267
        Width = 57
        Height = 10
        Caption = 'C'#243'digo de Baixa'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel138: TRLLabel
        Left = 643
        Top = 296
        Width = 80
        Height = 10
        Caption = 'Autentica'#231#227'o Mec'#226'nica'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel10: TRLLabel
        Left = 474
        Top = 144
        Width = 5
        Height = 10
        Caption = 'x'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        Transparent = False
      end
      object RLDBText18: TRLDBText
        Left = 570
        Top = 280
        Width = 204
        Height = 13
        AutoSize = False
        DataField = 'NossoNumero'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel16: TRLLabel
        Left = 644
        Top = 322
        Width = 81
        Height = 10
        Caption = 'Ficha de Compensa'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel19: TRLLabel
        Left = 0
        Top = 55
        Width = 28
        Height = 10
        Caption = 'Parcela'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel36: TRLLabel
        Left = 66
        Top = 55
        Width = 41
        Height = 10
        Caption = 'Vencimento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLDraw66: TRLDraw
        Left = 55
        Top = 53
        Width = 1
        Height = 30
        Angle = 90.000000000000000000
        DrawKind = dkLine
      end
      object RLLabel38: TRLLabel
        Left = 0
        Top = 83
        Width = 91
        Height = 10
        Caption = 'Ag'#234'ncia / C'#243'digo Cedente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel39: TRLLabel
        Left = 0
        Top = 107
        Width = 29
        Height = 10
        Caption = 'Esp'#233'cie'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel40: TRLLabel
        Left = 0
        Top = 117
        Width = 42
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Caption = 'R$'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel41: TRLLabel
        Left = 58
        Top = 107
        Width = 40
        Height = 10
        Caption = 'Quantidade'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLDraw67: TRLDraw
        Left = 55
        Top = 106
        Width = 1
        Height = 27
        Angle = 90.000000000000000000
        DrawKind = dkLine
      end
      object RLLabel42: TRLLabel
        Left = 0
        Top = 133
        Width = 87
        Height = 10
        Caption = '( = ) Valor do Documento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel43: TRLLabel
        Left = 0
        Top = 158
        Width = 92
        Height = 10
        Caption = '( - ) Desconto / Abatimento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel44: TRLLabel
        Left = 0
        Top = 185
        Width = 62
        Height = 10
        Caption = '( + ) Mora / Multa'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel45: TRLLabel
        Left = 0
        Top = 212
        Width = 68
        Height = 10
        Caption = '( = ) Valor Cobrado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLDraw68: TRLDraw
        Left = 0
        Top = 184
        Width = 141
        Height = 1
        DrawKind = dkLine
      end
      object RLDraw69: TRLDraw
        Left = 0
        Top = 211
        Width = 141
        Height = 1
        DrawKind = dkLine
      end
      object RLLabel46: TRLLabel
        Left = 0
        Top = 241
        Width = 53
        Height = 10
        Caption = 'Nosso N'#250'mero'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLDraw70: TRLDraw
        Left = 0
        Top = 266
        Width = 141
        Height = 1
        DrawKind = dkLine
      end
      object RLLabel47: TRLLabel
        Left = 0
        Top = 268
        Width = 27
        Height = 10
        Caption = 'Sacado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel48: TRLLabel
        Left = 0
        Top = 330
        Width = 63
        Height = 10
        Caption = 'Recibo do Sacado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLLabel49: TRLLabel
        Left = 0
        Top = 341
        Width = 68
        Height = 10
        Caption = 'Autenticar no Verso'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -8
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object RLDraw71: TRLDraw
        Left = 140
        Top = 12
        Width = 11
        Height = 353
        Angle = 90.000000000000000000
        DrawKind = dkLine
        Pen.Style = psDot
        Transparent = False
      end
      object ImgLoja: TRLImage
        Left = 0
        Top = 12
        Width = 143
        Height = 41
        Center = True
        Scaled = True
      end
      object txtTotPar: TRLLabel
        Left = 29
        Top = 67
        Width = 13
        Height = 14
        Caption = '/X'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object mIntrucoes: TRLMemo
        Left = 152
        Top = 170
        Width = 410
        Height = 64
        AutoSize = False
        Behavior = [beSiteExpander]
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtVencCanhoto: TRLLabel
        Left = 56
        Top = 67
        Width = 82
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object txtCodCedenteCarne: TRLLabel
        Left = 5
        Top = 91
        Width = 129
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtValorCarne: TRLLabel
        Left = 0
        Top = 143
        Width = 137
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object txtNossoNumeroCarne: TRLLabel
        Left = 0
        Top = 251
        Width = 137
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtCPF: TRLLabel
        Left = 0
        Top = 315
        Width = 142
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtLocal: TRLLabel
        Left = 154
        Top = 65
        Width = 412
        Height = 14
        Hint = 'Teste'
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtNomeCedente: TRLLabel
        Left = 154
        Top = 92
        Width = 412
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtDataDocto: TRLLabel
        Left = 152
        Top = 118
        Width = 84
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtNumeroDocto: TRLLabel
        Left = 247
        Top = 118
        Width = 94
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtEspecieDoc: TRLLabel
        Left = 347
        Top = 118
        Width = 68
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtAceite: TRLLabel
        Left = 418
        Top = 118
        Width = 56
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtDataProces: TRLLabel
        Left = 479
        Top = 118
        Width = 86
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtCarteira: TRLLabel
        Left = 247
        Top = 143
        Width = 50
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtNomeSacado: TRLLabel
        Left = 180
        Top = 241
        Width = 386
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtEndSacado: TRLLabel
        Left = 180
        Top = 254
        Width = 386
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtCidadeSacado: TRLLabel
        Left = 180
        Top = 267
        Width = 386
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtVencCarne2: TRLLabel
        Left = 572
        Top = 67
        Width = 172
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object txtNossoNumCan: TRLLabel
        Left = 572
        Top = 118
        Width = 172
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtValorCar: TRLLabel
        Left = 572
        Top = 143
        Width = 172
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object txtNomeSacadoCarne: TRLLabel
        Left = 0
        Top = 281
        Width = 144
        Height = 16
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtCodCedenteCarne2: TRLLabel
        Left = 572
        Top = 92
        Width = 172
        Height = 13
        Alignment = taRightJustify
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtCPFCarne2: TRLLabel
        Left = 570
        Top = 251
        Width = 204
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtParcela: TRLLabel
        Left = -26
        Top = 67
        Width = 51
        Height = 14
        Alignment = taRightJustify
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtRazaoSocial: TRLLabel
        Left = 258
        Top = 15
        Width = 412
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtLogradouro: TRLLabel
        Left = 258
        Top = 28
        Width = 412
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtCidade: TRLLabel
        Left = 258
        Top = 40
        Width = 412
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtCnpj: TRLLabel
        Left = 154
        Top = 328
        Width = 412
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtEmail: TRLLabel
        Left = 154
        Top = 346
        Width = 412
        Height = 13
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object txtNomeCont: TRLLabel
        Left = -1
        Top = 298
        Width = 144
        Height = 16
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
    end
  end
  object RLPDFFilter1: TRLPDFFilter
    DocumentInfo.Creator = 
      'FortesReport (Open Source) v3.24(B14)  \251 Copyright '#169' 1999-200' +
      '8 Fortes Inform'#225'tica'
    DisplayName = 'ACBrBoleto - http://acbr.sf.net'
    Left = 128
    Top = 324
  end
  object RLHTMLFilter1: TRLHTMLFilter
    DocumentStyle = dsCSS2
    DisplayName = 'ACBrBoleto - http://acbr.sf.net'
    Left = 158
    Top = 324
  end
  object ds_Cr: TDataSource
    DataSet = scdsCr_proc
    Left = 832
    Top = 459
  end
  object scdsCr_proc: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    Params = <
      item
        DataType = ftInteger
        Name = 'pcod'
        ParamType = ptInput
      end>
    ProviderName = 'DataSetProvider1'
    Left = 800
    Top = 460
    object scdsCr_procTITULO: TStringField
      FieldName = 'TITULO'
      ProviderFlags = [pfInUpdate]
      Required = True
      Size = 18
    end
    object scdsCr_procEMISSAO: TDateField
      FieldName = 'EMISSAO'
      ProviderFlags = [pfInUpdate]
    end
    object scdsCr_procDATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object scdsCr_procCAIXA: TSmallintField
      FieldName = 'CAIXA'
      ProviderFlags = [pfInUpdate]
    end
    object scdsCr_procSTATUS: TStringField
      DisplayWidth = 20
      FieldName = 'STATUS'
      ProviderFlags = [pfInUpdate]
      Required = True
      FixedChar = True
      Size = 16
    end
    object scdsCr_procN_DOCUMENTO: TStringField
      FieldName = 'N_DOCUMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object scdsCr_procVALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object scdsCr_procVALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object scdsCr_procVALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object scdsCr_procNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      ProviderFlags = []
      Required = True
      Size = 50
    end
    object scdsCr_procVALORREC: TFloatField
      FieldName = 'VALORREC'
      ProviderFlags = []
      DisplayFormat = ',#0.00'
    end
    object scdsCr_procCODRECEBIMENTO: TIntegerField
      FieldName = 'CODRECEBIMENTO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object scdsCr_procDP: TIntegerField
      FieldName = 'DP'
      ProviderFlags = [pfInUpdate]
    end
    object scdsCr_procVALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
      ProviderFlags = [pfInUpdate]
      ReadOnly = True
    end
    object scdsCr_procCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object scdsCr_procTIT: TStringField
      FieldName = 'TIT'
      ReadOnly = True
      FixedChar = True
      Size = 254
    end
    object scdsCr_procSITUACAO: TStringField
      FieldName = 'SITUACAO'
      ReadOnly = True
      Required = True
      FixedChar = True
      Size = 2
    end
    object scdsCr_procFORMARECEBIMENTO: TStringField
      FieldName = 'FORMARECEBIMENTO'
      ProviderFlags = [pfInUpdate]
      FixedChar = True
      Size = 1
    end
    object scdsCr_procDATARECEBIMENTO: TDateField
      FieldName = 'DATARECEBIMENTO'
      ReadOnly = True
    end
    object scdsCr_procCODVENDA: TIntegerField
      FieldName = 'CODVENDA'
      ProviderFlags = [pfInUpdate]
    end
    object scdsCr_procVIA: TStringField
      FieldName = 'VIA'
      Required = True
      FixedChar = True
      Size = 4
    end
    object scdsCr_procTRecebido: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TRecebido'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'SUM(VALORRECEBIDO)'
    end
    object scdsCr_procTotal_resto: TAggregateField
      Alignment = taRightJustify
      FieldName = 'Total_resto'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(VALOR_PRIM_VIA-VALORRECEBIDO)'
    end
    object scdsCr_procTotalTitulo: TAggregateField
      FieldName = 'TotalTitulo'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(VALOR_PRIM_VIA)'
    end
  end
  object DataSetProvider1: TDataSetProvider
    DataSet = SQLDataSet1
    Options = [poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 768
    Top = 460
  end
  object SQLDataSet1: TSQLDataSet
    CommandText = 
      'select rec.CODVENDA, rec.CODCLIENTE, rec.CODRECEBIMENTO, rec.TIT' +
      'ULO, rec.VIA, rec.DATAVENCIMENTO, rec.VALORTITULO, rec.DP, '#13#10'   ' +
      '       CASE rec.STATUS WHEN '#39'5-'#39' then '#39'Pendente'#39' when '#39'7-'#39' then ' +
      #39'Recebido'#39' when '#39'8-'#39' then '#39'Credito Duvidoso'#39' when '#39'9-'#39' then '#39'Em ' +
      'Cobran'#231'a'#39' when '#39'14'#39' then '#39'Cancelado'#39' end  as STATUS, '#13#10'         ' +
      ' rec.VALORRECEBIDO, rec.VALOR_RESTO, rec.VALOR_PRIM_VIA, rec.N_D' +
      'OCUMENTO, rec.CAIXA, rec.FORMARECEBIMENTO, rec.DATARECEBIMENTO,'#13 +
      #10'          cli.NOMECLIENTE, (CASE WHEN (REC.VALORRECEBIDO > 0) T' +
      'HEN (REC.VALORTITULO - rec.VALOR_RESTO)'#13#10'          WHEN (REC.VAL' +
      'ORRECEBIDO = 0) THEN rec.VALOR_RESTO END)'#13#10'          as VALORREC' +
      ', rec.EMISSAO, (udf_digits(rec.TITULO) || '#39'/'#39' || rec.VIA) as TIT' +
      ', rec.STATUS as SITUACAO '#13#10'from RECEBIMENTO rec '#13#10'           inn' +
      'er join CLIENTES cli on cli.CODCLIENTE=rec.CODCLIENTE '#13#10'where re' +
      'c.CODVENDA = :pcod order by rec.CODRECEBIMENTO'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'pcod'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 736
    Top = 460
    object SQLDataSet1TITULO: TStringField
      FieldName = 'TITULO'
      ProviderFlags = [pfInUpdate]
      Required = True
      Size = 18
    end
    object SQLDataSet1DATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1CAIXA: TSmallintField
      FieldName = 'CAIXA'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1STATUS: TStringField
      DisplayWidth = 20
      FieldName = 'STATUS'
      ProviderFlags = [pfInUpdate]
      Required = True
      FixedChar = True
      Size = 16
    end
    object SQLDataSet1N_DOCUMENTO: TStringField
      FieldName = 'N_DOCUMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1VALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1VALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1VALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1VALORREC: TFloatField
      FieldName = 'VALORREC'
      ProviderFlags = []
    end
    object SQLDataSet1CODRECEBIMENTO: TIntegerField
      FieldName = 'CODRECEBIMENTO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object SQLDataSet1NOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      ProviderFlags = []
      ReadOnly = True
      Required = True
      Size = 50
    end
    object SQLDataSet1DP: TIntegerField
      FieldName = 'DP'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1EMISSAO: TDateField
      FieldName = 'EMISSAO'
      ProviderFlags = [pfInUpdate]
      ReadOnly = True
    end
    object SQLDataSet1VALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
      ProviderFlags = [pfInUpdate]
      ReadOnly = True
    end
    object SQLDataSet1CODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object SQLDataSet1TIT: TStringField
      FieldName = 'TIT'
      ReadOnly = True
      FixedChar = True
      Size = 254
    end
    object SQLDataSet1SITUACAO: TStringField
      FieldName = 'SITUACAO'
      ReadOnly = True
      Required = True
      FixedChar = True
      Size = 2
    end
    object SQLDataSet1FORMARECEBIMENTO: TStringField
      FieldName = 'FORMARECEBIMENTO'
      ProviderFlags = [pfInUpdate]
      FixedChar = True
      Size = 1
    end
    object SQLDataSet1DATARECEBIMENTO: TDateField
      FieldName = 'DATARECEBIMENTO'
      ReadOnly = True
    end
    object SQLDataSet1CODVENDA: TIntegerField
      FieldName = 'CODVENDA'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1VIA: TStringField
      FieldName = 'VIA'
      Required = True
      FixedChar = True
      Size = 4
    end
  end
  object total_parc: TSQLDataSet
    CommandText = 'select count(VIA) from RECEBIMENTO '#13#10'where CODVENDA = :idv'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'idv'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 832
    Top = 508
    object total_parcCOUNT: TIntegerField
      FieldName = 'COUNT'
      ReadOnly = True
      Required = True
    end
  end
  object buscaCli: TSQLDataSet
    CommandText = 
      'SELECT a.*,'#13#10'               e.*,  e.numero numer'#13#10'   FROM CLIENT' +
      'ES a'#13#10'   left outer join ENDERECOCLIENTE e on e.CODCLIENTE = a.C' +
      'ODCLIENTE '#13#10'where a.CODCLIENTE = :idcli'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'idcli'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 832
    Top = 548
    object buscaCliCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object buscaCliNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object buscaCliBLOQUEIO: TStringField
      FieldName = 'BLOQUEIO'
      FixedChar = True
      Size = 1
    end
    object buscaCliLOGRADOURO: TStringField
      FieldName = 'LOGRADOURO'
      Size = 50
    end
    object buscaCliTELEFONE: TStringField
      FieldName = 'TELEFONE'
      Size = 11
    end
    object buscaCliRAZAOSOCIAL: TStringField
      FieldName = 'RAZAOSOCIAL'
      Required = True
      Size = 50
    end
    object buscaCliCONTATO: TStringField
      FieldName = 'CONTATO'
      Size = 30
    end
    object buscaCliTIPOFIRMA: TSmallintField
      FieldName = 'TIPOFIRMA'
      Required = True
    end
    object buscaCliCPF: TStringField
      FieldName = 'CPF'
      Size = 14
    end
    object buscaCliCNPJ: TStringField
      FieldName = 'CNPJ'
      Size = 18
    end
    object buscaCliINSCESTADUAL: TStringField
      FieldName = 'INSCESTADUAL'
      Size = 24
    end
    object buscaCliRG: TStringField
      FieldName = 'RG'
      Size = 14
    end
    object buscaCliSEGMENTO: TSmallintField
      FieldName = 'SEGMENTO'
      Required = True
    end
    object buscaCliREGIAO: TSmallintField
      FieldName = 'REGIAO'
      Required = True
    end
    object buscaCliLIMITECREDITO: TFloatField
      FieldName = 'LIMITECREDITO'
    end
    object buscaCliDATACADASTRO: TDateField
      FieldName = 'DATACADASTRO'
      Required = True
    end
    object buscaCliCODUSUARIO: TIntegerField
      FieldName = 'CODUSUARIO'
      Required = True
    end
    object buscaCliSTATUS: TSmallintField
      FieldName = 'STATUS'
      Required = True
    end
    object buscaCliHOMEPAGE: TStringField
      FieldName = 'HOMEPAGE'
      Size = 40
    end
    object buscaCliPRAZORECEBIMENTO: TSmallintField
      FieldName = 'PRAZORECEBIMENTO'
    end
    object buscaCliPRAZOENTREGA: TSmallintField
      FieldName = 'PRAZOENTREGA'
    end
    object buscaCliCODBANCO: TSmallintField
      FieldName = 'CODBANCO'
    end
    object buscaCliBASE_ICMS: TSmallintField
      FieldName = 'BASE_ICMS'
    end
    object buscaCliDATANASC: TDateField
      FieldName = 'DATANASC'
    end
    object buscaCliCONTA_CLIENTE: TStringField
      FieldName = 'CONTA_CLIENTE'
      Size = 15
    end
    object buscaCliOBS: TStringField
      FieldName = 'OBS'
      Size = 200
    end
    object buscaCliSEXO: TStringField
      FieldName = 'SEXO'
      FixedChar = True
      Size = 1
    end
    object buscaCliFORMA_CORRESPOND: TStringField
      FieldName = 'FORMA_CORRESPOND'
      Size = 30
    end
    object buscaCliGRUPO_CLIENTE: TStringField
      FieldName = 'GRUPO_CLIENTE'
      Size = 30
    end
    object buscaCliCODINCLUCANC: TIntegerField
      FieldName = 'CODINCLUCANC'
    end
    object buscaCliEXIST_COBERT: TStringField
      FieldName = 'EXIST_COBERT'
      Size = 6
    end
    object buscaCliEXISTCOPART: TStringField
      FieldName = 'EXISTCOPART'
      Size = 6
    end
    object buscaCliDATAREINC: TDateField
      FieldName = 'DATAREINC'
    end
    object buscaCliGERAAVISO: TStringField
      FieldName = 'GERAAVISO'
      FixedChar = True
      Size = 1
    end
    object buscaCliGERAENV: TStringField
      FieldName = 'GERAENV'
      FixedChar = True
      Size = 1
    end
    object buscaCliGERABOL: TStringField
      FieldName = 'GERABOL'
      FixedChar = True
      Size = 1
    end
    object buscaCliDTAALTERA: TDateField
      FieldName = 'DTAALTERA'
    end
    object buscaCliSERIE: TStringField
      FieldName = 'SERIE'
      Size = 4
    end
    object buscaCliCURSO: TStringField
      FieldName = 'CURSO'
      Size = 50
    end
    object buscaCliMARCA: TStringField
      FieldName = 'MARCA'
      Size = 60
    end
    object buscaCliCOD_FAIXA: TIntegerField
      FieldName = 'COD_FAIXA'
    end
    object buscaCliDESCONTO: TFloatField
      FieldName = 'DESCONTO'
    end
    object buscaCliPARCELA: TIntegerField
      FieldName = 'PARCELA'
    end
    object buscaCliPARCELAGERADAS: TIntegerField
      FieldName = 'PARCELAGERADAS'
    end
    object buscaCliNUMERO: TIntegerField
      FieldName = 'NUMERO'
    end
    object buscaCliTRANSPORTE: TStringField
      FieldName = 'TRANSPORTE'
      Size = 50
    end
    object buscaCliDIVERSO1: TStringField
      FieldName = 'DIVERSO1'
      Size = 50
    end
    object buscaCliDIVERSO2: TStringField
      FieldName = 'DIVERSO2'
      Size = 50
    end
    object buscaCliPERIODO: TStringField
      FieldName = 'PERIODO'
      Size = 15
    end
    object buscaCliFOTO: TStringField
      FieldName = 'FOTO'
      Size = 300
    end
    object buscaCliCFOP: TStringField
      FieldName = 'CFOP'
      FixedChar = True
      Size = 4
    end
    object buscaCliCOD_CLI: TStringField
      FieldName = 'COD_CLI'
      Size = 10
    end
    object buscaCliCODENDERECO: TIntegerField
      FieldName = 'CODENDERECO'
    end
    object buscaCliCODCLIENTE_1: TIntegerField
      FieldName = 'CODCLIENTE_1'
    end
    object buscaCliBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Size = 30
    end
    object buscaCliCOMPLEMENTO: TStringField
      FieldName = 'COMPLEMENTO'
      Size = 30
    end
    object buscaCliCIDADE: TStringField
      FieldName = 'CIDADE'
      Size = 40
    end
    object buscaCliUF: TStringField
      FieldName = 'UF'
      FixedChar = True
      Size = 2
    end
    object buscaCliCEP: TStringField
      FieldName = 'CEP'
      Size = 10
    end
    object buscaCliTELEFONE1: TStringField
      FieldName = 'TELEFONE1'
      Size = 9
    end
    object buscaCliTELEFONE2: TStringField
      FieldName = 'TELEFONE2'
      Size = 9
    end
    object buscaCliFAX: TStringField
      FieldName = 'FAX'
      Size = 9
    end
    object buscaCliE_MAIL: TStringField
      FieldName = 'E_MAIL'
      Size = 200
    end
    object buscaCliRAMAL: TStringField
      FieldName = 'RAMAL'
      Size = 5
    end
    object buscaCliTIPOEND: TSmallintField
      FieldName = 'TIPOEND'
    end
    object buscaCliDADOSADICIONAIS: TStringField
      FieldName = 'DADOSADICIONAIS'
      Size = 200
    end
    object buscaCliDDD: TStringField
      FieldName = 'DDD'
      Size = 3
    end
    object buscaCliDDD1: TStringField
      FieldName = 'DDD1'
      Size = 3
    end
    object buscaCliDDD2: TStringField
      FieldName = 'DDD2'
      Size = 3
    end
    object buscaCliDDD3: TStringField
      FieldName = 'DDD3'
      Size = 3
    end
    object buscaCliNUMERO_1: TStringField
      FieldName = 'NUMERO_1'
      Size = 5
    end
    object buscaCliCD_IBGE: TStringField
      FieldName = 'CD_IBGE'
      Size = 10
    end
    object buscaCliPAIS: TStringField
      FieldName = 'PAIS'
      Size = 60
    end
    object buscaCliNUMER: TStringField
      FieldName = 'NUMER'
      Size = 5
    end
  end
  object s_parametro: TSQLDataSet
    CommandText = 'select * from PARAMETRO '#13#10'where PARAMETRO = :pr'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'pr'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 836
    Top = 597
    object s_parametroDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 100
    end
    object s_parametroPARAMETRO: TStringField
      FieldName = 'PARAMETRO'
      Required = True
      Size = 40
    end
    object s_parametroCONFIGURADO: TStringField
      FieldName = 'CONFIGURADO'
      FixedChar = True
      Size = 1
    end
    object s_parametroDADOS: TStringField
      FieldName = 'DADOS'
      Size = 40
    end
    object s_parametroD1: TStringField
      FieldName = 'D1'
      Size = 30
    end
    object s_parametroD2: TStringField
      FieldName = 'D2'
      Size = 30
    end
    object s_parametroD3: TStringField
      FieldName = 'D3'
      Size = 30
    end
    object s_parametroD4: TStringField
      FieldName = 'D4'
      Size = 30
    end
    object s_parametroD5: TStringField
      FieldName = 'D5'
      Size = 30
    end
    object s_parametroD6: TStringField
      FieldName = 'D6'
      Size = 30
    end
    object s_parametroD7: TStringField
      FieldName = 'D7'
      Size = 30
    end
    object s_parametroD8: TStringField
      FieldName = 'D8'
      Size = 30
    end
    object s_parametroD9: TStringField
      FieldName = 'D9'
      Size = 30
    end
    object s_parametroINSTRUCOES: TStringField
      FieldName = 'INSTRUCOES'
      Size = 200
    end
    object s_parametroVALOR: TFloatField
      FieldName = 'VALOR'
    end
  end
end

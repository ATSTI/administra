...
class hr_analytic_timesheet(osv.osv):
    ...
    _columns = {
        ...
        'manager_id': fields.related('account_id', 'user_id', type='many2one',string='GERENTE', relation='res.users', store=True),
    }
    ...
    def create(self, cr, uid, vals, context=None):
        if context is None:
            context = {}
        man_obj = self.pool.get('account.analytic.account')
        man_id = man_obj.search(cr, uid, [('id', '=', vals.get('account_id'))], context=context)
        if man_id:
            for manager_idx in man_obj.browse(cr, uid, man_id, context=context):
                if not vals.get('manager_id',False):
                    vals.update(manager_id=manager_idx.manager_id.id)
                if not vals.get('partner_id',False):
                    vals.update(partner_id=manager_idx.partner_id.id)
        emp_obj = self.pool.get('hr.employee')
        emp_id = emp_obj.search(cr, uid, [('user_id', '=', context.get('user_id') or uid)], context=context)
        ename = ''
        if emp_id:

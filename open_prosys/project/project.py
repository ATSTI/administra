...
class project(osv.osv):
    ...
    _columns = {
        ...
        'contrato': fields.char('Número Contrato', size=64, select=True),
...
class task(base_stage, osv.osv):
    ...
    _columns = {
        ...
        'grupo': fields.char('Grupo', size=60, required=True, select=True),
        'categoria': fields.char('Categoria', size=60, required=True,select=True),
        'atividade': fields.char('Cod. Atividade', size=20, required=True,select=True),
        'ordserv': fields.char('Ord. Serviço', size=40, required=True,select=True),
    }
    _defaults = {
...
class project_work(osv.osv):
    ...
    _columns = {
        ...
        'detalhe': fields.text('Descricao'),
        'attachment_ids': fields.many2many('ir.attachment','apontamento_docs','work_id', 'attachment_id', 'Attachments',help="Adicione os documentos"),
        'date': fields.date('Data Entrada', select="1"),
        'date_base': fields.date('Data Base', select="1"),
        'date_out': fields.date('Data Saída', select="1"),
        'task_id': fields.many2one('project.task', 'Task', ondelete='cascade', required=True, select="1"),
        'hours': fields.float('Time Spent'),
        'hours_in': fields.float('Entrada'),
        'hours_out': fields.float('Saída'),
        'feriado': fields.boolean('Feriado', readonly=False),
        'periculosidade': fields.boolean('Periculosidade', readonly=False),
        'retrabalho': fields.boolean('Retrabalho', readonly=False),
        'justmedica': fields.boolean('Just. Médica', readonly=False),
        'justespecial': fields.boolean('Just. Especial', readonly=False),
        'externo': fields.boolean('Serv. Externo', readonly=False, select="1"),
        'sobreaviso': fields.boolean('Sobreaviso', readonly=False),
        'embarcado': fields.boolean('Embarcado', readonly=False),
        'adicional1': fields.char('Adicional 1', size=128),
        'adicional2': fields.char('Adicional 2', size=128),
        'adicional3': fields.char('Adicional 3', size=128),
        'adicional4': fields.char('Adicional 4', size=128),
    ...
    _defaults = {
        ...
        'date_base': lambda *a: time.strftime('%Y-%m-%d'),
        'date_out': lambda *a: time.strftime('%Y-%m-%d'),
    }
